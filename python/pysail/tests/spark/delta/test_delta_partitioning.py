...
class TestDeltaPartitioning:
    ...
    def test_delta_partitioning_by_single_column(self, spark, tmp_path):
        ...
        assert partition_dirs == {"year=2025", "year=2026"}, (
            f"Expected year partitions, got {partition_dirs}"
        )
        ...
        expected_data = pd.DataFrame(
            ...
            {"id": "int32", "event": "string", "score": "float64", "year": "int32"}
        )
        ...
        expected = 2025
        expected_filtered = (
            expected_data[expected_data["year"] == expected]
            .sort_values("id")
        ...
        assert filtered_df.count() == 2, (
            "Partition pruning should return exactly 2 records for year=2025"
        )  # noqa: PLR2004

        filtered_df_ne = (
            spark.read.format("delta").load(delta_table_path).filter("year != 2025")
        )
        assert filtered_df_ne.count() == 2, (
            "NOT EQUAL filter should return 2 records for year!=2025"
        )  # noqa: PLR2004

        filtered_df_gt = (
            spark.read.format("delta").load(delta_table_path).filter("year > 2025")
        )
        assert filtered_df_gt.count() == 2, (
            "GREATER THAN filter should return 2 records for year>2025"
        )  # noqa: PLR2004

    def test_delta_partitioning_creates_correct_directory_structure(
        self, spark, delta_test_data, tmp_path
    ):
        ...
        df.write.format("delta").mode("overwrite").partitionBy("id").save(
            str(delta_path)
        )
        ...
        assert actual_partitions == expected_partitions, (
            f"Expected {expected_partitions}, got {actual_partitions}"
        )
    ...
    def test_delta_partitioning_by_multiple_columns(self, spark, tmp_path):
        ...
        expected_data = pd.DataFrame(
            ...
            {"id": "int32", "value": "int32", "region": "int32", "category": "int32"}
        )
        ...
        df_region1 = (
            spark.read.format("delta").load(f"{delta_path}").filter("region = 1")
        )
        ...
        df_region2_cat2 = (
            spark.read.format("delta")
            .load(f"{delta_path}")
            .filter("region = 2 AND category = 2")
        )
        assert df_region2_cat2.count() == 1, "Region 2, Category 2 should have 1 record"

        df_region_ge2 = (
            spark.read.format("delta").load(f"{delta_path}").filter("region >= 2")
        )
