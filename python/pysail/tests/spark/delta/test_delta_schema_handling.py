...
class TestDeltaSchemaHandling:
    ...
    def test_delta_schema_read_with_custom_schema(self, spark, tmp_path):
        ...
        spark.createDataFrame(data, schema=schema).write.format("delta").mode(
            "overwrite"
        ).save(str(delta_path))
        ...
        result_df = (
            ...
            .filter("name IS NOT NULL AND score > 80")
        )
        ...
        assert loaded_schema == schema, (
            f"Schema mismatch: expected {schema}, got {loaded_schema}"
        )
    ...
    def test_delta_schema_merge_and_overwrite_fails_together(self, spark, tmp_path):
        ...
        with pytest.raises(Exception, match=r"(?i).*merge.*overwrite.*"):
            df2.write.format("delta").mode("append").option(
                "mergeSchema", "true"
            ).option("overwriteSchema", "true").save(str(delta_path))
