...
def test_delta_pruning_with_equality(spark, tmp_path):
...
def test_delta_pruning_with_comparison(spark, tmp_path):
...
def test_delta_pruning_with_in_operator(spark, tmp_path):
...
def test_delta_pruning_with_between_operator(spark, tmp_path):
    ...
    for year in range(2020, 2025):
        for month in range(1, 13):
            partition_data.append(Row(id=len(partition_data), year=year, month=month, value=f"data_{year}_{month:02d}"))
...
def test_delta_pruning_with_null_handling(spark, tmp_path):
...
def test_delta_pruning_with_null_string_partition_round_trips_as_null(spark, tmp_path):
...
def test_delta_pruning_with_string_partitions(spark, tmp_path):
...
def test_delta_pruning_with_edge_cases(spark, tmp_path):
...
def test_delta_pruning_with_no_matching_partitions(spark, tmp_path):
...
def test_delta_pruning_with_mixed_types(spark, tmp_path):
...
def test_delta_pruning_correctness(spark, tmp_path):
...
def test_delta_pruning_with_non_partition_columns(spark, tmp_path):
...
def setup_partitioned_table(spark, tmp_path_factory):
...
def test_delta_pruning_parametrized_scenarios(spark, setup_partitioned_table, filter_str, expected_count):
...
def test_delta_pruning_basic_scenarios(spark, tmp_path, filter_condition, expected_count, description):
