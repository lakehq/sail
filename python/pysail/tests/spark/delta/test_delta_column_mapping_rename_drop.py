...
def test_column_mapping_rename_preserves_physical(spark, tmp_path: Path):
...
def test_column_mapping_drop_column_retains_ids(spark, tmp_path: Path):
...
def test_column_mapping_overwrite_preserves_custom_metadata(spark, tmp_path: Path):
