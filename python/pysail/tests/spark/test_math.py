...
@pytest.mark.skipif(
    platform.system() == "Windows", reason="`time.tzset()` is not available on Windows"
)
def test_plus(spark, session_timezone, local_timezone):  # noqa: ARG001
    ...
    assert spark.sql(
        "SELECT DATE'2025-02-26' + INTERVAL '2' MONTH AS result"
    ).collect() == [Row(result=datetime.date(2025, 4, 26))]
    assert spark.sql(
        "SELECT TIMESTAMP'2025-02-26 12:15:29' + INTERVAL '3' SECOND AS result"
    ).collect() == [
        Row(result=datetime.datetime(2025, 2, 26, 12, 15, 32))  # noqa: DTZ001
    ]
    assert spark.sql(
        "SELECT DATE'2025-03-31' + INTERVAL '1' MONTH AS result"
    ).collect() == [Row(result=datetime.date(2025, 4, 30))]
    assert spark.sql("SELECT DATE'2025-02-26' + 1 AS result").collect() == [
        Row(result=datetime.date(2025, 2, 27))
    ]
    assert spark.sql("SELECT 1 + DATE'2025-02-26' AS result").collect() == [
        Row(result=datetime.date(2025, 2, 27))
    ]
...
def test_minus(spark, session_timezone, local_timezone):  # noqa: ARG001
    ...
    assert spark.sql(
        "SELECT DATE'2025-03-31' - INTERVAL '1' MONTH AS result"
    ).collect() == [Row(result=datetime.date(2025, 2, 28))]
    assert spark.sql("SELECT DATE'2025-02-26' - 1 AS result").collect() == [
        Row(result=datetime.date(2025, 2, 25))
    ]
...
def test_multiply(spark):
    ...
    assert spark.sql(
        "SELECT INTERVAL '3:15' HOUR TO MINUTE * 3 AS result"
    ).collect() == [Row(result=datetime.timedelta(seconds=35100))]
    assert spark.sql("SELECT INTERVAL '3' YEAR * 3 AS result").collect() == [
        Row(result=108)
    ]
...
def test_divide(spark):
    ...
    assert spark.sql("SELECT 6 / Decimal(3) AS result").collect() == [
        Row(result=Decimal("2.00000000000"))
    ]
    ...
    assert spark.sql(
        "SELECT INTERVAL '3:15' HOUR TO MINUTE / 3 AS result"
    ).collect() == [Row(result=datetime.timedelta(seconds=3900))]
...
def test_div(spark):
    ...
    if not is_jvm_spark():
        pytest.skip("the following tests are not supported in Sail yet")
    assert spark.sql(
        "SELECT INTERVAL '10' DAY div INTERVAL '3' DAY AS result"
    ).collect() == [Row(result=3)]
    assert spark.sql(
        "SELECT INTERVAL '1-1' YEAR TO MONTH div INTERVAL '-1' MONTH AS result"
    ).collect() == [Row(result=-13)]
    assert spark.sql(
        "SELECT INTERVAL '100' HOUR div INTERVAL '1' DAY AS result"
    ).collect() == [Row(result=4)]
