...
def test_insert_multiple_values(spark):
    spark.sql(
        "INSERT INTO person VALUES (201, 'Shehab', 99), (202, 'Alice', 2), (203, 'Bob', 9000)"
    )
    actual = spark.sql("SELECT * FROM person WHERE id IN (201, 202, 203)").toPandas()
    expected = pd.DataFrame(
        {
            "id": [201, 202, 203],
            "name": ["Shehab", "Alice", "Bob"],
            "age": [99, 2, 9000],
        },
    ).astype({"id": "int32", "name": "str", "age": "int32"})
    assert_frame_equal(actual, expected)
...
def test_insert_using_column_list(spark):
    spark.sql(
        "INSERT INTO person (id, name, age) VALUES (301, 'Shehab', 99), (302, 'Alice', 2), (303, 'Bob', 9000)"
    )
    ...
    spark.sql(
        "INSERT INTO person (name, age, id) VALUES ('Shehab', 99, 321), ('Alice', 2, 322), ('Bob', 9000, 323)"
    )
...
def test_delete_entire_table(spark):
    ...
    spark.sql(
        "INSERT INTO meow VALUES (601, 'Shehab', 99), (602, 'Alice', 2), (603, 'Bob', 9000)"
    )
    spark.sql("DELETE FROM meow")
    actual = spark.sql("SELECT * FROM meow").toPandas()
    expected = pd.DataFrame(columns=["id", "name", "age"]).astype(
        {"id": "int32", "name": "str", "age": "int32"}
    )
    assert_frame_equal(actual, expected)
...
def test_update_multiple_values(spark):
    spark.sql(
        "INSERT INTO person VALUES (901, 'Shehab', 99), (902, 'Alice', 2), (903, 'Bob', 9000)"
    )
...
def test_update_with_full_table_name(spark):
    ...
    spark.sql(
        "UPDATE spark_catalog.default.person SET id = 1002, age = 100 WHERE id = 1001"
    )
...
def test_update_with_full_table_name_and_alias(spark):
    ...
    spark.sql(
        "UPDATE spark_catalog.default.person AS p SET p.id = 1202, p.age = 100 WHERE p.id = 1201"
    )
