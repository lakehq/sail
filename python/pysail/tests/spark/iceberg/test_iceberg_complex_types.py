...
@pytest.mark.skip(
    reason="pyiceberg client does not support list literals for default values"
)
...
def test_schema_evolution_nested_type_promotion(spark, sql_catalog):
    ...
    schema = Schema(
        ...
        NestedField(
            ...
            name="data",
            field_type=ListType(
                element_id=3, element=IntegerType(), element_required=False
            ),
            required=False,
        ),
    )
    ...
    try:
        ...
        df_long.write.format("iceberg").option("mergeSchema", "true").mode(
            "append"
        ).save(table.location())
...
def test_iceberg_merge_schema_nested_struct_promotes_float_to_double(
    spark, sql_catalog
):
    ...
    schema = Schema(
        ...
        NestedField(
            ...
            field_type=StructType(
                NestedField(
                    field_id=3, name="latitude", field_type=FloatType(), required=False
                ),
                NestedField(
                    field_id=4, name="longitude", field_type=FloatType(), required=False
                ),
            ),
            required=False,
        ),
...
def test_iceberg_merge_schema_map_key_promotion(spark, sql_catalog):
    ...
    try:
        ...
        with pytest.raises(
            Exception, match=r"(?i)Schema evolution for map keys is not allowed"
        ):
            (
                evolved_df.write.format("iceberg")
                .mode("append")
...
def test_iceberg_merge_schema_map_key_invalid_promotion(spark, sql_catalog):
    ...
    try:
        ...
        with pytest.raises(
            Exception,
            match=r"(?i)Column 'key' has type.*Set mergeSchema=true to allow schema evolution",
        ):
            (
                evolved_df.write.format("iceberg")
                .mode("append")
...
def test_pruning_exotic_types(spark, sql_catalog):
    ...
    try:
        ...
        filtered = (
            spark.read.format("iceberg").load(table.location()).filter("f_val > 5.0")
        )
