...
def test_iceberg_time_travel_by_snapshot_id(spark, tmp_path):
    ...
    try:
        ...
        assert_frame_equal(
            latest,
            pd.DataFrame({"id": [1, 2], "value": ["v0", "v1"]}).astype(latest.dtypes),
        )

        tt_df = (
            spark.read.format("iceberg")
            .option("snapshotId", str(sid0))
            .load(table.location())
            .sort("id")
            .toPandas()
        )
        assert_frame_equal(
            tt_df, pd.DataFrame({"id": [1], "value": ["v0"]}).astype(tt_df.dtypes)
        )
    finally:
...
def test_iceberg_time_travel_by_timestamp(spark, tmp_path):
    ...
    try:
        ...
        v0 = [Row(id=1, value="v0")]
        spark.createDataFrame(v0).write.format("iceberg").mode("overwrite").save(
            table_location
        )
        ...
        v1 = [Row(id=2, value="v1")]
        spark.createDataFrame(v1).write.format("iceberg").mode("append").save(
            table_location
        )
        ...
        spark.createDataFrame(v2).write.format("iceberg").mode("append").save(
            table_location
        )
...
def test_iceberg_time_travel_precedence_snapshot_over_timestamp(spark, tmp_path):
    ...
    try:
        spark.createDataFrame([Row(id=1, value="old")]).write.format("iceberg").mode(
            "overwrite"
        ).save(table.location())
        table.refresh()
        ...
        spark.createDataFrame([Row(id=2, value="new")]).write.format("iceberg").mode(
            "overwrite"
        ).save(table.location())
...
def test_iceberg_time_travel_by_ref_main_if_available(spark, tmp_path):
    ...
    try:
        spark.createDataFrame([Row(id=1, value="x")]).write.format("iceberg").mode(
            "overwrite"
        ).save(table.location())
        try:
            df = (
...
def test_iceberg_time_travel_across_merge_schema(spark, tmp_path):
    ...
    try:
        ...
        tt_df = (
            spark.read.format("iceberg")
            .option("snapshotId", str(snapshot_id))
            .load(table.location())
            .orderBy("id")
        )
