@spark-4
Feature: try_make_interval
  Safe variant of make_interval that returns NULL on overflow or
  argument-type errors instead of throwing.

  Rule: Valid argument combinations build interval

    @sail-bug
    Scenario: All zeros yields zero interval
      When query
      """
      SELECT CAST(try_make_interval(0, 0, 0, 0, 0, 0, 0) AS STRING) AS result
      """
      Then query result
      | result    |
      | 0 seconds |

    @sail-bug
    Scenario: Year only
      When query
      """
      SELECT CAST(try_make_interval(1) AS STRING) AS result
      """
      Then query result
      | result  |
      | 1 years |

    @sail-bug
    Scenario: Year and month
      When query
      """
      SELECT CAST(try_make_interval(1, 6) AS STRING) AS result
      """
      Then query result
      | result            |
      | 1 years 6 months |

    @sail-bug
    Scenario: Year, month, week, day
      When query
      """
      SELECT CAST(try_make_interval(1, 2, 3, 4) AS STRING) AS result
      """
      Then query result
      | result                    |
      | 1 years 2 months 25 days |

    @sail-bug
    Scenario: Full args with hours, mins, fractional secs
      When query
      """
      SELECT CAST(try_make_interval(0, 1, 0, 1, 0, 0, 100.000001) AS STRING) AS result
      """
      Then query result
      | result                                          |
      | 1 months 1 days 1 minutes 40.000001 seconds   |

    @sail-bug
    Scenario: Negative values
      When query
      """
      SELECT CAST(try_make_interval(-1, -1, -1, -1, -1, -1, -1.5) AS STRING) AS result
      """
      Then query result
      | result                                                          |
      | -1 years -1 months -8 days -1 hours -1 minutes -1.5 seconds |

    @sail-bug
    Scenario: NULL field propagates to NULL result
      When query
      """
      SELECT try_make_interval(100, NULL, 3) AS result
      """
      Then query result
      | result |
      | NULL   |

  Rule: Overflow returns NULL instead of error

    @sail-bug
    Scenario: Year overflow returns NULL
      When query
      """
      SELECT try_make_interval(2147483647) AS result
      """
      Then query result
      | result |
      | NULL   |

  Rule: Argument count validation matches Spark

    @sail-bug
    Scenario: No args raises WRONG_NUM_ARGS
      When query
      """
      SELECT try_make_interval()
      """
      Then query error WRONG_NUM_ARGS|calendar_interval|Unsupported

  Rule: Multi-row batches

    @sail-bug
    Scenario: Mixed rows including overflow
      When query
      """
      SELECT CAST(try_make_interval(y) AS STRING) AS result FROM VALUES (0), (1), (CAST(NULL AS INT)), (2147483647) AS t(y)
      """
      Then query result
      | result    |
      | 0 seconds |
      | 1 years   |
      | NULL      |
      | NULL      |

    @sail-bug
    Scenario: Per-row args from columns
      When query
      """
      SELECT CAST(try_make_interval(y, m, w, d) AS STRING) AS result FROM VALUES
        (1, 2, 3, 4),
        (0, 0, 0, 0),
        (10, 5, 1, 2) AS t(y, m, w, d)
      """
      Then query result
      | result                   |
      | 1 years 2 months 25 days |
      | 0 seconds                |
      | 10 years 5 months 9 days |

    @sail-bug
    Scenario: Per-row args with NULL field propagates per-row
      When query
      """
      SELECT CAST(try_make_interval(y, m, w) AS STRING) AS result FROM VALUES
        (1, 1, 1),
        (1, CAST(NULL AS INT), 1),
        (2, 2, 2) AS t(y, m, w)
      """
      Then query result
      | result                  |
      | 1 years 1 months 7 days |
      | NULL                    |
      | 2 years 2 months 14 days |

  # Spark 4.2.0 intervalExpressions.scala `TryMakeInterval` is RuntimeReplaceable by
  # `MakeInterval(..., failOnError = false)`, so its `nullable` is always true, and it only
  # swallows the ArithmeticException of IntervalUtils.makeInterval: the implicit cast of
  # `secs` to DECIMAL(18, 6) still fails under ANSI.
  Rule: try_make_interval edges

    @sail-bug
    Scenario: try_make_interval is nullable even for non-null literals
      When query
        """
        SELECT try_make_interval(1, 2) AS result
        """
      Then query schema
        """
        root
         |-- result: interval (nullable = true)
        """

    @sail-bug
    Scenario: try_make_interval keeps a time part that fits in Long microseconds
      When query
        """
        SELECT try_make_interval(0, 0, 0, 0, h, mi) AS result
        FROM VALUES (2147483647, 2147483647), (1, 2) AS t(h, mi)
        """
      Then query result
        | result                     |
        | 2183275041 hours 7 minutes |
        | 1 hours 2 minutes          |

    @sail-bug
    Scenario: try_make_interval still fails the seconds cast to DECIMAL(18, 6) under ANSI
      Given config spark.sql.ansi.enabled = true
      When query
        """
        SELECT try_make_interval(0, 0, 0, 0, 0, 0, 1000000000000.5) AS result
        """
      Then query error \[NUMERIC_VALUE_OUT_OF_RANGE\.WITH_SUGGESTION\]

    @sail-bug
    Scenario: try_make_interval seconds outside DECIMAL(18, 6) yield NULL without ANSI
      Given config spark.sql.ansi.enabled = false
      When query
        """
        SELECT try_make_interval(0, 0, 0, 0, 0, 0, s) AS result
        FROM VALUES (1000000000000.5), (1.0000005) AS t(s)
        """
      Then query result
        | result           |
        | NULL             |
        | 1.000001 seconds |
