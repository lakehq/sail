Feature: to_unix_timestamp with an argument coming from a column
  ...
  Rule: to_unix_timestamp truncates sub-second instants toward zero

    Background:
      Given config spark.sql.session.timeZone = UTC

    @sail-bug
    Scenario Outline: to_unix_timestamp truncates a parsed pre-epoch fraction toward zero: <case>
      When query
        """
        SELECT to_unix_timestamp('<value>', 'yyyy-MM-dd HH:mm:ss.S') AS result
        """
      Then query result
        | result   |
        | <result> |
    ...
    @sail-bug
    Scenario: to_unix_timestamp truncates parsed fractions toward zero from a column
      When query
        """
        SELECT to_unix_timestamp(c, 'yyyy-MM-dd HH:mm:ss.S') AS result
    ...
    Scenario: to_unix_timestamp truncates TIMESTAMP fractions toward zero from a column
      When query
        """
        SELECT to_unix_timestamp(c) AS result
  ...
  Rule: to_unix_timestamp rejects out-of-range fields per ANSI mode

    Background:
      Given config spark.sql.session.timeZone = UTC

    Scenario Outline: to_unix_timestamp rejects <case> under ANSI
      Given config spark.sql.ansi.enabled = true
      When query
        """
        SELECT to_unix_timestamp('<value>', 'yyyy-MM-dd') AS result
    ...
    Scenario Outline: to_unix_timestamp returns NULL for <case> without ANSI
      Given config spark.sql.ansi.enabled = false
      When query
        """
        SELECT to_unix_timestamp('<value>', 'yyyy-MM-dd') AS result
    ...
    @sail-bug
    Scenario: to_unix_timestamp rejects a numeric argument with Spark's type-mismatch error
      When query
        """
        SELECT to_unix_timestamp(123) AS result
        """
  ...
  Rule: to_unix_timestamp interprets local values in the session time zone

    Scenario Outline: to_unix_timestamp of string, DATE, TIMESTAMP and TIMESTAMP_NTZ in the session zone <zone>
      Given config spark.sql.session.timeZone = <zone>
      When query
        """
        SELECT
          to_unix_timestamp('2024-01-15 01:02:03') AS str,
          to_unix_timestamp(DATE '2024-07-15') AS dt,
          to_unix_timestamp(TIMESTAMP '2024-01-15 01:02:03') AS ltz,
          to_unix_timestamp(TIMESTAMP_NTZ '2024-01-15 01:02:03') AS ntz
        """
      Then query result
        | str   | dt   | ltz   | ntz        |
    ...
    Scenario Outline: to_unix_timestamp parses string rows in the session zone <zone>
      Given config spark.sql.session.timeZone = <zone>
      When query
        """
        SELECT to_unix_timestamp(c, 'yyyy-MM-dd HH:mm') AS result
    ...
    Scenario Outline: to_unix_timestamp converts DATE rows in the session zone <zone>
      Given config spark.sql.session.timeZone = <zone>
      When query
        """
        SELECT to_unix_timestamp(c) AS result
    ...
    Scenario: to_unix_timestamp resolves Los Angeles DST transitions row by row
      Given config spark.sql.session.timeZone = America/Los_Angeles
      When query
        """
        SELECT to_unix_timestamp(c) AS result
    ...
    Scenario: to_unix_timestamp resolves Chatham DST transitions
      Given config spark.sql.session.timeZone = Pacific/Chatham
      When query
        """
        SELECT to_unix_timestamp('2024-09-29 03:00:00') AS gap, to_unix_timestamp('2024-04-07 03:00:00') AS overlap
        """
      Then query result
        | gap        | overlap    |
