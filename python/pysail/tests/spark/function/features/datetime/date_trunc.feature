Feature: DATE_TRUNC output type
  ...
  Rule: a timestamp before the nanosecond range truncates like any other

    # The mirror of "date boundaries and leap days": Spark 4.2.0 DateTimeUtils.truncTimestamp works
    # on microseconds and even MICROSECOND/SECOND never touch a zone. Sail's truncation goes through
    # nanoseconds and fails for anything before 1677-09-21 00:12:43, at every granularity.

    Background:
      Given config spark.sql.session.timeZone = UTC

    Scenario: date_trunc just inside the nanosecond range
      When query
        """
        SELECT
          date_trunc('DAY', TIMESTAMP '1677-09-22 05:00:00') AS day,
          date_trunc('DAY', TIMESTAMP '1900-01-01 05:00:00') AS day_1900
        """
      Then query result
        | day                 | day_1900            |
        | 1677-09-22 00:00:00 | 1900-01-01 00:00:00 |

    @sail-bug
    Scenario Outline: date_trunc before the nanosecond range: <case>
      When query
        """
        SELECT date_trunc('<unit>', <value>) AS result
        """
      Then query result
        | result   |
        | <result> |
      ...
      Examples:
        | case                           | unit        | value                                     | result                     |
        | hour in year 1                 | HOUR        | TIMESTAMP '0001-01-01 05:30:00'           | 0001-01-01 05:00:00        |
        | minute in year 1               | MINUTE      | TIMESTAMP '0001-01-01 05:30:10'           | 0001-01-01 05:30:00        |
        | second in year 1               | SECOND      | TIMESTAMP '0001-01-01 05:30:00.5'         | 0001-01-01 05:30:00        |
        | microsecond at the minimum     | MICROSECOND | TIMESTAMP '0001-01-01 00:00:00.000001'    | 0001-01-01 00:00:00.000001 |
        | week in year 1                 | WEEK        | TIMESTAMP '0001-01-02 00:00:00'           | 0001-01-01 00:00:00        |
        | year in year 1                 | YEAR        | TIMESTAMP '0001-06-01 00:00:00'           | 0001-01-01 00:00:00        |
        | day in year 1500               | DAY         | TIMESTAMP '1500-06-15 05:30:00'           | 1500-06-15 00:00:00        |
        | month on the bound day         | MONTH       | TIMESTAMP '1677-09-21 00:12:44'           | 1677-09-01 00:00:00        |
        | year of a timestamp_ntz        | YEAR        | TIMESTAMP_NTZ '1500-06-15 10:00:00'       | 1500-01-01 00:00:00        |

    @sail-bug
    Scenario: date_trunc of distinct timestamps on both sides of the nanosecond range from a column
      When query
        """
        SELECT id, date_trunc('MONTH', ts) AS result
        FROM VALUES
          (1, TIMESTAMP '0001-06-15 10:00:00'),
          (2, TIMESTAMP '1500-06-15 10:00:00'),
          (3, TIMESTAMP '2024-06-15 10:00:00')
          AS t(id, ts)
        ORDER BY id
        """
      Then query result ordered
        | id | result              |
        | 1  | 0001-06-01 00:00:00 |
        | 2  | 1500-06-01 00:00:00 |
