Feature: dayofyear output schema
  ...
  Rule: Output schema
    ...
    @sail-bug
    Scenario: a non-null timestamp_ntz literal input to dayofyear is nullable through the cast to DATE
      When query
        """
        SELECT dayofyear(TIMESTAMP_NTZ '2024-01-01 00:00:00') AS result
        """
      Then query schema
        """
        root
