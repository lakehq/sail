Feature: nullability of an explicit CAST
  ...
  Rule: scalar casts, force-nullable
    ...
    @sail-bug
    Scenario Outline: a scalar CAST that is force-nullable: <case> (known Sail bug)
      When query
        """
        SELECT CAST(<input> AS <type>) AS result
        """
  ...
  Rule: scalar casts, force-nullable, VARIANT and INTERVAL types
    ...
    @spark-4
    @sail-bug
    Scenario Outline: a scalar CAST that is force-nullable: <case>, on Spark 4+ (known Sail bug)
      When query
        """
        SELECT CAST(<input> AS <type>) AS result
        """
