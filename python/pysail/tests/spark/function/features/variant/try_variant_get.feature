Feature: try_variant_get with an argument coming from a column
  ...
  Rule: Output schema
    ...
    Scenario: a non-null literal input to try_variant_get yields the schema Spark declares
      When query
        """
        SELECT try_variant_get(parse_json('{"a": 1}'), '$.a', 'int') AS result
        """
      Then query schema
        """
        root
    ...
    Scenario: a nullable column input to try_variant_get stays nullable
      When query
        """
        SELECT try_variant_get(c, '$.a', 'int') AS result FROM VALUES (parse_json('{"a": 1}')), (CAST(NULL AS VARIANT)) AS t(c)
        """
      Then query schema
        """
        root
