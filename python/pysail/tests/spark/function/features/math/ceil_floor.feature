Feature: ceil() and floor() round numbers toward +/- infinity
  ...
  Rule: the target scale may be any foldable expression
    # Spark evaluates the scale during analysis, so it accepts any foldable expression,
    # not just a bare literal. Sail folds it while building the plan, since the function
    # can only read a scale that reaches it as a literal. A scale that references a
    # column is not foldable and Spark rejects it outright.

    Scenario: ceil with a foldable scale expression
      When query
        """
        SELECT ceil(CAST(3.14159 AS DECIMAL(10,5)), 1 + 1) AS result,
               typeof(ceil(CAST(3.14159 AS DECIMAL(10,5)), 1 + 1)) AS t
        """
      Then query result
        | result | t            |
        | 3.15   | decimal(8,2) |

    Scenario: floor with a foldable scale expression
      When query
        """
        SELECT floor(CAST(3.14159 AS DECIMAL(10,5)), 1 + 1) AS result,
               typeof(floor(CAST(3.14159 AS DECIMAL(10,5)), 1 + 1)) AS t
        """
      Then query result
        | result | t            |
        | 3.14   | decimal(8,2) |

    Scenario: ceil with a negative foldable scale expression
      When query
        """
        SELECT ceil(CAST(1234.5 AS DECIMAL(10,1)), 0 - 2) AS result
        """
      Then query result
        | result |
        | 1300   |
    ...
    @sail-bug
    Scenario: ceil rejects a scale taken from a column
      # Both engines reject it, but Spark names the DATATYPE_MISMATCH class while Sail
      # reports "Target scale must be Integer literal".
      When query
        """
        SELECT ceil(CAST(3.14159 AS DECIMAL(10,5)), c) AS result
        FROM VALUES (1, 2) AS t(i, c)
        """
      Then query error NON_FOLDABLE_ARGUMENT

    @sail-bug
    Scenario: floor rejects a scale taken from a column
      When query
        """
        SELECT floor(CAST(3.14159 AS DECIMAL(10,5)), c) AS result
        FROM VALUES (1, 2) AS t(i, c)
        """
      Then query error NON_FOLDABLE_ARGUMENT
