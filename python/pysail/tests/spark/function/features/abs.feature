...
Feature: abs comprehensive tests
  ...
  Rule: idempotence simplify (abs(abs(x)) = abs(x))
    # SparkAbs::simplify detects nested same-name calls and collapses them.
    # Mathematically valid because |x| >= 0 always, so |abs(x)| = abs(x).
    # Safe under both ANSI modes — the inner abs already errored or wrapped.
