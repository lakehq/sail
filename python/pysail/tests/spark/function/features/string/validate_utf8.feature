Feature: validate_utf8 output schema
  ...
  Rule: Atomic non-string input is cast to string
    ...
    Scenario Outline: atomic input: <case>
      When query
        """
        SELECT validate_utf8(<argument>) AS result
        """
  ...
  Rule: Binary input
    ...
    Scenario: valid UTF-8 bytes pass through
      When query
        """
        SELECT validate_utf8(X'61') AS result
        """
      Then query result collected
        | result |
        | a      |

    Scenario: invalid UTF-8 bytes raise
      When query
        """
        SELECT validate_utf8(X'80') AS result
        """
      Then query error (?i)(INVALID_UTF8_STRING|non UTF-8)
  ...
  Rule: Non-atomic input is rejected
    ...
    Scenario: a string reached through a container is accepted
      When query
        """
        SELECT validate_utf8(element_at(array('a'), 1)) AS result
        """
      Then query result collected
        | result |
        | a      |
  ...
  Rule: Default output column name
    ...
    # Tagged for the same root cause as the Output schema Rule above: `validate_utf8` desugars to
    # `cast(arg, Utf8)`, which inherits the argument's nullability, so a non-null literal reports
    # `nullable = false` where Spark hardcodes `true`. The NAME half of these assertions already
    # matches; it is only the nullability in the tree that fails.
    @sail-bug
    Scenario Outline: the column name comes from the function name and its argument: <case>
      When query
        """
        SELECT validate_utf8(<argument>)
        """
      Then query schema
        """
        root
      ...
      Examples:
        ...
        | string literal | 'abc'    | validate_utf8(abc)        | string |
        | binary literal | X'61'    | validate_utf8(X'61')      | string |
        | empty string   | ''       | validate_utf8()           | string |
    ...
    # Spark `validate_utf8(abc)`, Sail `VALIDATE_UTF8(abc)`. Sail-wide behaviour, not specific to this function.
    @sail-bug
    Scenario: the column name is lower-cased regardless of how the call was spelled
      ...
      Then query schema
        """
        root
         |-- validate_utf8(abc): string (nullable = true)
        """
  ...
  Rule: Composes with other expressions
    ...
    Scenario: the result can be collected into an array
      When query
        """
        SELECT array(validate_utf8('a'), validate_utf8(X'62')) AS result
        """
      Then query result collected
        | result   |
        | ['a', 'b'] |
    ...
    Scenario: the result can be consumed by another string function
      When query
        """
        SELECT upper(validate_utf8('ab')) AS result
        """
      Then query result collected
        | result |
        | AB     |
    ...
    Scenario: a nullable column is resolved row by row
      When query
        """
        SELECT validate_utf8(c) AS result FROM VALUES ('a'), (CAST(NULL AS STRING)) AS t(c)
        """
      Then query result collected
        | result |
        | a      |
