Feature: sequence output schema
  ...
  Rule: Integral execution semantics
    ...
    Scenario: sequence keeps lazy evaluation inside a filter predicate
      When query
        """
        SELECT id
        FROM VALUES
        ...
        WHERE sequence(
          ...
          CASE
            WHEN start IS NULL
              THEN CAST(raise_error('filtered-null') AS BIGINT)
            ELSE stop
          END
        ) IS NULL
        """
    ...
    # rand(seed) restarts when row-wise lazy evaluation invokes it once per active row.
    @sail-bug
