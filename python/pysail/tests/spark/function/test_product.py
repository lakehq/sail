...
def test_product_rejects_boolean(spark):
    # Spark rejects boolean input with DATATYPE_MISMATCH — `product` requires a DOUBLE — and
    # does so in both ANSI modes, since it is an analysis type check rather than an ANSI gate.
    # Sail coerces boolean to 1.0/0.0 and returns a value instead.
