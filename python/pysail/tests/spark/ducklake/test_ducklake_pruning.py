...
def _attach(conn: duckdb.DuckDBPyConnection, meta_path: str, data_path: str, schema_name: str = "dl"):
    conn.execute(f"ATTACH 'ducklake:sqlite:{meta_path}' AS {schema_name} (DATA_PATH '{data_path}/')")
...
def _append_batches(conn: duckdb.DuckDBPyConnection, table_ident: str, batches: list[pd.DataFrame]):
    for df in batches:
        ...
        conn.execute(f"INSERT INTO {table_ident} SELECT * FROM tmp_append")  # noqa: S608
...
def _mk_table_null_bool(conn: duckdb.DuckDBPyConnection, meta_path, data_path, ident="dl.prune_null_bool"):
    ...
    try:
        ...
        _append_batches(
            ...
            [
                pd.DataFrame(
                    [{"id": 1, "region": None, "active": True}, {"id": 2, "region": None, "active": True}]
                ).astype({"id": "int64"}),
                pd.DataFrame(
                    [{"id": 3, "region": "US", "active": False}, {"id": 4, "region": "EU", "active": False}]
                ).astype({"id": "int64"}),
            ],
        )
    finally:
...
def _add_identity_partition(
    conn: duckdb.DuckDBPyConnection, meta_path: str, data_path: str, table_name: str, src_col: str
):
    _attach(conn, meta_path, data_path)
    try:
        meta_schema = "__ducklake_metadata_dl"
        table_id = conn.execute(
            f"SELECT table_id FROM {meta_schema}.ducklake_table WHERE table_name = ? AND end_snapshot IS NULL",  # noqa: S608
            [table_name],
        ).fetchone()[0]
        col_row = conn.execute(
            f"SELECT column_id FROM {meta_schema}.ducklake_column WHERE table_id = ? AND column_name = ? AND end_snapshot IS NULL",  # noqa: S608
            [table_id, src_col],
        ).fetchone()
        ...
        conn.execute(
            f"INSERT INTO {meta_schema}.ducklake_partition_info(partition_id, table_id, begin_snapshot, end_snapshot) VALUES (?, ?, 0, NULL)",  # noqa: S608
            [partition_id, table_id],
        )
        conn.execute(
            f"INSERT INTO {meta_schema}.ducklake_partition_column(partition_id, table_id, partition_key_index, column_id, transform) VALUES (?, ?, 0, ?, 'identity')",  # noqa: S608
            [partition_id, table_id, column_id],
        )

        files = conn.execute(
            f"SELECT data_file_id, file_order FROM {meta_schema}.ducklake_data_file WHERE table_id = ? AND end_snapshot IS NULL ORDER BY file_order",  # noqa: S608
            [table_id],
        ).fetchall()
        n = len(files)
        for i, (fid, _order) in enumerate(files):
            ...
            conn.execute(
                f"INSERT INTO {meta_schema}.ducklake_file_partition_value(data_file_id, table_id, partition_key_index, partition_value) VALUES (?, ?, 0, ?)",  # noqa: S608
                [fid, table_id, val],
            )
    finally:
        _detach(conn)
...
def test_pruning_equality_filters(spark, duckdb_conn, ducklake_paths):
    ...
    assert df.filter("year = 2023").count() == 4  # noqa: PLR2004
    assert df.filter("year = 2023 AND month = 1").count() == 2  # noqa: PLR2004
...
def test_pruning_in_clause(spark, duckdb_conn, ducklake_paths):
    ...
    assert df.filter("month IN (2)").count() == 4  # noqa: PLR2004
...
def test_comparison_and_between(spark, duckdb_conn, ducklake_paths):
    ...
    assert df.filter("year > 2022").count() == 6  # noqa: PLR2004
    assert df.filter("year BETWEEN 2022 AND 2023").count() == 6  # noqa: PLR2004
    assert df.filter("year >= 2023 AND month >= 6").count() == 4  # noqa: PLR2004
...
def test_null_and_boolean(spark, duckdb_conn, ducklake_paths):
    ...
    assert df.filter("region IS NULL").count() == 2  # noqa: PLR2004
    assert df.filter("active = true").count() == 2  # noqa: PLR2004
...
def test_string_in_and_range_pruning(spark, duckdb_conn, ducklake_paths):
    ...
    assert df.filter("team IN ('backend','frontend')").count() == 2  # noqa: PLR2004
    assert df.filter("dept > 'engineering'").count() == 2  # noqa: PLR2004
...
def test_limit_pushdown_behavior(spark, duckdb_conn, ducklake_paths):
    ...
    assert df.filter("flag = true").limit(7).count() == 7  # noqa: PLR2004
...
def test_identity_partition_range_pruning(spark, duckdb_conn, ducklake_paths):
    ...
    assert df.filter("year >= 2024").count() == 4  # noqa: PLR2004
    assert df.filter("year BETWEEN 2023 AND 2024").count() == 8  # noqa: PLR2004
