...
def test_ducklake_read_with_sql_location(spark, ducklake_setup):
...
def test_ducklake_read_with_schema_prefix(spark, tmp_path, duckdb_conn):
...
def test_ducklake_read_empty_table(spark, tmp_path, duckdb_conn):
...
def test_ducklake_read_various_types(spark, tmp_path, duckdb_conn):
...
def test_ducklake_read_missing_table_error(spark, tmp_path, duckdb_conn):
...
def test_ducklake_read_missing_options_error(spark):
...
def test_ducklake_read_with_join(spark, ducklake_setup, duckdb_conn):
...
def test_ducklake_read_count(spark, ducklake_setup):
...
def test_ducklake_pg_basic_read(spark, ducklake_pg_setup):
    postgres_url, data_path, table = ducklake_pg_setup
    df = spark.read.format("ducklake").options(url=postgres_url, table=table, base_path=f"file://{data_path}/").load()
    result = df.sort("id").collect()
    expected_count = 3
    assert len(result) == expected_count
    assert result[0].name == "Alice"
...
def test_ducklake_pg_with_filter(spark, ducklake_pg_setup):
    postgres_url, data_path, table = ducklake_pg_setup
    df = spark.read.format("ducklake").options(url=postgres_url, table=table, base_path=f"file://{data_path}/").load()
...
def test_ducklake_pg_with_projection(spark, ducklake_pg_setup):
    postgres_url, data_path, table = ducklake_pg_setup
    df = spark.read.format("ducklake").options(url=postgres_url, table=table, base_path=f"file://{data_path}/").load()
...
def test_ducklake_pg_with_limit(spark, ducklake_pg_setup):
    postgres_url, data_path, table = ducklake_pg_setup
    df = spark.read.format("ducklake").options(url=postgres_url, table=table, base_path=f"file://{data_path}/").load()
    limit_count = 2
    assert df.limit(limit_count).count() == limit_count


def test_ducklake_pg_read_to_pandas(spark, ducklake_pg_setup):
    postgres_url, data_path, table = ducklake_pg_setup
    df = spark.read.format("ducklake").options(url=postgres_url, table=table, base_path=f"file://{data_path}/").load()
    pdf = df.sort("id").toPandas()
    expected_count = 3
    assert len(pdf) == expected_count
    assert list(pdf["name"]) == ["Alice", "Bob", "Charlie"]


def test_ducklake_pg_with_sql_location(spark, ducklake_pg_setup):
