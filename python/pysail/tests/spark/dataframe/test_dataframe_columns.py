...
def _rows(df):
    """The rows as text, with the client's own rendering of a value normalised away.

    PySpark 3.5 hands a BINARY value back as `bytearray` and 4.x as `bytes`, which is the client
    talking about itself rather than the engine, so the two have to read the same here.
    """
    return sorted(
        str({k: bytes(v) if isinstance(v, bytearray) else v for k, v in row.asDict().items()})
...
RESULTS = [
    ...
    ('with_columns_entries_differing_in_case', 'true', ['a', 'b', 'c', 'C'], ["{'a': 1, 'b': 2, 'c': 1, 'C': 2}"], [{}, {}, {}, {}]),
...
def test_replacement_survives_a_later_analysis(spark, case_sensitive, columns):
    ...
    try:
        ...
        assert df.columns == columns
        assert df.groupBy("product").count().count() == 2
    finally:
        _unconfigure(spark)
...
def _composition(spark):
    ...
    COMPOSITION = {
        ...
        "replaced_then_select_old_case": lambda: base().withColumn("A", col("a") + 10).select("a"),
        "replaced_then_filter_old_case": lambda: base().withColumn("A", col("a") + 10).filter(col("a") > 10),
        "replaced_then_order_by_old_case": lambda: base().withColumn("A", col("a") + 10).orderBy("a"),
        "replaced_then_drop_old_case": lambda: base().withColumn("A", col("a") + 10).drop("a"),
        "replaced_then_aggregate_old_case": lambda: base().withColumn("A", col("a") + 10).agg(spark_sum("a").alias("s")),
        ...
        "replaced_then_window": lambda: (
            base()
            .withColumn("A", col("a"))
            .withColumn("n", row_number().over(Window.partitionBy("a").orderBy("b")))
        ),
...
COMPOSITION_RESULTS = [
    ...
    ('replaced_then_filter_old_case', 'false', ['A', 'b'], ["{'A': 11, 'b': 'x'}", "{'A': 11, 'b': 'z'}", "{'A': 12, 'b': 'y'}"]),
    ('replaced_then_filter_old_case', 'true', ['a', 'b', 'A'], []),
    ('replaced_then_order_by_old_case', 'false', ['A', 'b'], ["{'A': 11, 'b': 'x'}", "{'A': 11, 'b': 'z'}", "{'A': 12, 'b': 'y'}"]),
    ('replaced_then_order_by_old_case', 'true', ['a', 'b', 'A'], ["{'a': 1, 'b': 'x', 'A': 11}", "{'a': 1, 'b': 'z', 'A': 11}", "{'a': 2, 'b': 'y', 'A': 12}"]),
    ('replaced_then_drop_old_case', 'false', ['b'], ["{'b': 'x'}", "{'b': 'y'}", "{'b': 'z'}"]),
    ('replaced_then_drop_old_case', 'true', ['b', 'A'], ["{'b': 'x', 'A': 11}", "{'b': 'y', 'A': 12}", "{'b': 'z', 'A': 11}"]),
    ('replaced_then_aggregate_old_case', 'false', ['s'], ["{'s': 34}"]),
    ('replaced_then_aggregate_old_case', 'true', ['s'], ["{'s': 4}"]),
    ('replaced_then_join_on_name', 'false', ['A', 'b', 'c'], ["{'A': 1, 'b': 'x', 'c': 'p'}", "{'A': 1, 'b': 'z', 'c': 'p'}"]),
    ('replaced_then_join_on_name', 'true', ['a', 'b', 'A', 'c'], ["{'a': 1, 'b': 'x', 'A': 1, 'c': 'p'}", "{'a': 1, 'b': 'z', 'A': 1, 'c': 'p'}"]),
    ('replaced_then_join_on_condition', 'false', ['A', 'b', 'a', 'c'], ["{'A': 1, 'b': 'x', 'a': 1, 'c': 'p'}", "{'A': 1, 'b': 'z', 'a': 1, 'c': 'p'}"]),
    ('replaced_then_join_on_condition', 'true', ['a', 'b', 'A', 'a', 'c'], ["{'a': 1, 'b': 'x', 'A': 1, 'c': 'p'}", "{'a': 1, 'b': 'z', 'A': 1, 'c': 'p'}"]),
    ('replaced_then_self_join', 'false', ['A', 'b', 'a', 'b'], ["{'A': 1, 'b': 'x', 'a': 1}", "{'A': 1, 'b': 'x', 'a': 1}", "{'A': 1, 'b': 'z', 'a': 1}", "{'A': 1, 'b': 'z', 'a': 1}", "{'A': 2, 'b': 'y', 'a': 2}"]),
    ('replaced_then_self_join', 'true', ['a', 'b', 'A', 'a', 'b'], ["{'a': 1, 'b': 'x', 'A': 1}", "{'a': 1, 'b': 'x', 'A': 1}", "{'a': 1, 'b': 'z', 'A': 1}", "{'a': 1, 'b': 'z', 'A': 1}", "{'a': 2, 'b': 'y', 'A': 2}"]),
    ('replaced_then_window', 'false', ['A', 'b', 'n'], ["{'A': 1, 'b': 'x', 'n': 1}", "{'A': 1, 'b': 'z', 'n': 2}", "{'A': 2, 'b': 'y', 'n': 1}"]),
    ('replaced_then_window', 'true', ['a', 'b', 'A', 'n'], ["{'a': 1, 'b': 'x', 'A': 1, 'n': 1}", "{'a': 1, 'b': 'z', 'A': 1, 'n': 2}", "{'a': 2, 'b': 'y', 'A': 2, 'n': 1}"]),
    ('replaced_then_union', 'false', ['A', 'b'], ["{'A': 1, 'b': 'x'}", "{'A': 1, 'b': 'x'}", "{'A': 1, 'b': 'z'}", "{'A': 1, 'b': 'z'}", "{'A': 2, 'b': 'y'}", "{'A': 2, 'b': 'y'}"]),
    ('replaced_then_union_by_name', 'false', ['A', 'b'], ["{'A': 1, 'b': 'x'}", "{'A': 1, 'b': 'x'}", "{'A': 1, 'b': 'z'}", "{'A': 1, 'b': 'z'}", "{'A': 2, 'b': 'y'}", "{'A': 2, 'b': 'y'}"]),
    ('replaced_then_distinct', 'false', ['A', 'b'], ["{'A': 1, 'b': 'x'}", "{'A': 1, 'b': 'y'}", "{'A': 1, 'b': 'z'}"]),
    ('replaced_then_distinct', 'true', ['a', 'b', 'A'], ["{'a': 1, 'b': 'x', 'A': 1}", "{'a': 1, 'b': 'z', 'A': 1}", "{'a': 2, 'b': 'y', 'A': 1}"]),
    ('replaced_then_drop_duplicates', 'false', ['A', 'b'], ["{'A': 1, 'b': 'x'}"]),
    ('replaced_then_drop_duplicates', 'true', ['a', 'b', 'A'], ["{'a': 1, 'b': 'x', 'A': 1}", "{'a': 2, 'b': 'y', 'A': 1}"]),
    ('replaced_twice', 'false', ['a', 'b'], ["{'a': 3, 'b': 'x'}", "{'a': 3, 'b': 'z'}", "{'a': 4, 'b': 'y'}"]),
    ('replaced_twice', 'true', ['a', 'b', 'A'], ["{'a': 3, 'b': 'x', 'A': 2}", "{'a': 3, 'b': 'z', 'A': 2}", "{'a': 4, 'b': 'y', 'A': 3}"]),
    ('renamed_then_replaced', 'false', ['z', 'b'], ["{'z': 9, 'b': 'x'}", "{'z': 9, 'b': 'y'}", "{'z': 9, 'b': 'z'}"]),
    ('renamed_then_replaced', 'true', ['Z', 'b', 'z'], ["{'Z': 1, 'b': 'x', 'z': 9}", "{'Z': 1, 'b': 'z', 'z': 9}", "{'Z': 2, 'b': 'y', 'z': 9}"]),
    ...
    ('renamed_twice_then_select', 'false', ['z', 'y'], ["{'z': 1, 'y': 'x'}", "{'z': 1, 'y': 'z'}", "{'z': 2, 'y': 'y'}"]),
...
METADATA_RESULTS = [
    ('meta/empty', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/empty', ['a', 'b'], 'struct<a:int,b:string>', [{}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/one key', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/one key', ['a', 'b'], 'struct<a:int,b:string>', [{'k': 'v'}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/several keys', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'i': 'z', 'j': 'w', 'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/several keys', ['a', 'b'], 'struct<a:int,b:string>', [{'i': 'z', 'j': 'w', 'k': 'v'}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/integer value', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': 1}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/integer value', ['a', 'b'], 'struct<a:int,b:string>', [{'k': 1}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/float value', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': 1.5}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/float value', ['a', 'b'], 'struct<a:int,b:string>', [{'k': 1.5}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/boolean value', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': True}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/boolean value', ['a', 'b'], 'struct<a:int,b:string>', [{'k': True}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/null value', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': None}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/null value', ['a', 'b'], 'struct<a:int,b:string>', [{'k': None}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/list value', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': [1, 2]}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/list value', ['a', 'b'], 'struct<a:int,b:string>', [{'k': [1, 2]}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/nested value', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': {'n': 1}}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/nested value', ['a', 'b'], 'struct<a:int,b:string>', [{'k': {'n': 1}}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/empty string value', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': ''}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/empty string value', ['a', 'b'], 'struct<a:int,b:string>', [{'k': ''}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/unicode key and value', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'ké': 'vä'}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/unicode key and value', ['a', 'b'], 'struct<a:int,b:string>', [{'ké': 'vä'}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/dotted key', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'a.b': 'v'}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/dotted key', ['a', 'b'], 'struct<a:int,b:string>', [{'a.b': 'v'}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/quoted value', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': 'a "quoted" value'}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/quoted value', ['a', 'b'], 'struct<a:int,b:string>', [{'k': 'a "quoted" value'}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/long value', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': ''}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/long value', ['a', 'b'], 'struct<a:int,b:string>', [{'k': ''}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('meta/reserved looking key', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'comment': 'c', '__CHAR_VARCHAR_TYPE_STRING': 'x'}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('meta-literal/reserved looking key', ['a', 'b'], 'struct<a:int,b:string>', [{'comment': 'c', '__CHAR_VARCHAR_TYPE_STRING': 'x'}, {}], ["{'a': 1, 'b': 'x'}"]),
    ('expr/int literal', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('expr-replace/int literal', ['a', 'b'], 'struct<a:int,b:string>', [{}, {}], ["{'a': 1, 'b': 'x'}", "{'a': 1, 'b': 'y'}"]),
    ('expr-meta/int literal', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 1}"]),
    ('expr/string literal', ['a', 'b', 'c'], 'struct<a:int,b:string,c:string>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': 's'}", "{'a': 2, 'b': 'y', 'c': 's'}"]),
    ('expr-replace/string literal', ['a', 'b'], 'struct<a:string,b:string>', [{}, {}], ["{'a': 's', 'b': 'x'}", "{'a': 's', 'b': 'y'}"]),
    ('expr-meta/string literal', ['a', 'b', 'c'], 'struct<a:int,b:string,c:string>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': 's'}", "{'a': 2, 'b': 'y', 'c': 's'}"]),
    ('expr/double literal', ['a', 'b', 'c'], 'struct<a:int,b:string,c:double>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': 1.5}", "{'a': 2, 'b': 'y', 'c': 1.5}"]),
    ('expr-replace/double literal', ['a', 'b'], 'struct<a:double,b:string>', [{}, {}], ["{'a': 1.5, 'b': 'x'}", "{'a': 1.5, 'b': 'y'}"]),
    ('expr-meta/double literal', ['a', 'b', 'c'], 'struct<a:int,b:string,c:double>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': 1.5}", "{'a': 2, 'b': 'y', 'c': 1.5}"]),
    ('expr/boolean literal', ['a', 'b', 'c'], 'struct<a:int,b:string,c:boolean>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': True}", "{'a': 2, 'b': 'y', 'c': True}"]),
    ('expr-replace/boolean literal', ['a', 'b'], 'struct<a:boolean,b:string>', [{}, {}], ["{'a': True, 'b': 'x'}", "{'a': True, 'b': 'y'}"]),
    ('expr-meta/boolean literal', ['a', 'b', 'c'], 'struct<a:int,b:string,c:boolean>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': True}", "{'a': 2, 'b': 'y', 'c': True}"]),
    ('expr/null literal', ['a', 'b', 'c'], 'struct<a:int,b:string,c:void>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': None}", "{'a': 2, 'b': 'y', 'c': None}"]),
    ('expr-replace/null literal', ['a', 'b'], 'struct<a:void,b:string>', [{}, {}], ["{'a': None, 'b': 'x'}", "{'a': None, 'b': 'y'}"]),
    ('expr-meta/null literal', ['a', 'b', 'c'], 'struct<a:int,b:string,c:void>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': None}", "{'a': 2, 'b': 'y', 'c': None}"]),
    ('expr/decimal cast', ['a', 'b', 'c'], 'struct<a:int,b:string,c:decimal(10,2)>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': Decimal('1.00')}", "{'a': 2, 'b': 'y', 'c': Decimal('1.00')}"]),
    ('expr-replace/decimal cast', ['a', 'b'], 'struct<a:decimal(10,2),b:string>', [{}, {}], ["{'a': Decimal('1.00'), 'b': 'x'}", "{'a': Decimal('1.00'), 'b': 'y'}"]),
    ('expr-meta/decimal cast', ['a', 'b', 'c'], 'struct<a:int,b:string,c:decimal(10,2)>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': Decimal('1.00')}", "{'a': 2, 'b': 'y', 'c': Decimal('1.00')}"]),
    ('expr/date cast', ['a', 'b', 'c'], 'struct<a:int,b:string,c:date>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': datetime.date(2024, 1, 15)}", "{'a': 2, 'b': 'y', 'c': datetime.date(2024, 1, 15)}"]),
    ('expr-replace/date cast', ['a', 'b'], 'struct<a:date,b:string>', [{}, {}], ["{'a': datetime.date(2024, 1, 15), 'b': 'x'}", "{'a': datetime.date(2024, 1, 15), 'b': 'y'}"]),
    ('expr-meta/date cast', ['a', 'b', 'c'], 'struct<a:int,b:string,c:date>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': datetime.date(2024, 1, 15)}", "{'a': 2, 'b': 'y', 'c': datetime.date(2024, 1, 15)}"]),
    ('expr/timestamp cast', ['a', 'b', 'c'], 'struct<a:int,b:string,c:timestamp>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': datetime.datetime(2024, 1, 15, 13, 0)}", "{'a': 2, 'b': 'y', 'c': datetime.datetime(2024, 1, 15, 13, 0)}"]),
    ('expr-replace/timestamp cast', ['a', 'b'], 'struct<a:timestamp,b:string>', [{}, {}], ["{'a': datetime.datetime(2024, 1, 15, 13, 0), 'b': 'x'}", "{'a': datetime.datetime(2024, 1, 15, 13, 0), 'b': 'y'}"]),
    ('expr-meta/timestamp cast', ['a', 'b', 'c'], 'struct<a:int,b:string,c:timestamp>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': datetime.datetime(2024, 1, 15, 13, 0)}", "{'a': 2, 'b': 'y', 'c': datetime.datetime(2024, 1, 15, 13, 0)}"]),
    ('expr/binary cast', ['a', 'b', 'c'], 'struct<a:int,b:string,c:binary>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': b's'}", "{'a': 2, 'b': 'y', 'c': b's'}"]),
    ('expr-replace/binary cast', ['a', 'b'], 'struct<a:binary,b:string>', [{}, {}], ["{'a': b's', 'b': 'x'}", "{'a': b's', 'b': 'y'}"]),
    ('expr-meta/binary cast', ['a', 'b', 'c'], 'struct<a:int,b:string,c:binary>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': b's'}", "{'a': 2, 'b': 'y', 'c': b's'}"]),
    ('expr/array', ['a', 'b', 'c'], 'struct<a:int,b:string,c:array<int>>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': [1, 2]}", "{'a': 2, 'b': 'y', 'c': [1, 2]}"]),
    ('expr-replace/array', ['a', 'b'], 'struct<a:array<int>,b:string>', [{}, {}], ["{'a': [1, 2], 'b': 'x'}", "{'a': [1, 2], 'b': 'y'}"]),
    ('expr-meta/array', ['a', 'b', 'c'], 'struct<a:int,b:string,c:array<int>>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': [1, 2]}", "{'a': 2, 'b': 'y', 'c': [1, 2]}"]),
    ('expr/map', ['a', 'b', 'c'], 'struct<a:int,b:string,c:map<string,int>>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': {'k': 1}}", "{'a': 2, 'b': 'y', 'c': {'k': 1}}"]),
    ('expr-replace/map', ['a', 'b'], 'struct<a:map<string,int>,b:string>', [{}, {}], ["{'a': {'k': 1}, 'b': 'x'}", "{'a': {'k': 1}, 'b': 'y'}"]),
    ('expr-meta/map', ['a', 'b', 'c'], 'struct<a:int,b:string,c:map<string,int>>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': {'k': 1}}", "{'a': 2, 'b': 'y', 'c': {'k': 1}}"]),
    ('expr/struct', ['a', 'b', 'c'], 'struct<a:int,b:string,c:struct<n:int>>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': Row(n=1)}", "{'a': 2, 'b': 'y', 'c': Row(n=1)}"]),
    ('expr-replace/struct', ['a', 'b'], 'struct<a:struct<n:int>,b:string>', [{}, {}], ["{'a': Row(n=1), 'b': 'x'}", "{'a': Row(n=1), 'b': 'y'}"]),
    ('expr-meta/struct', ['a', 'b', 'c'], 'struct<a:int,b:string,c:struct<n:int>>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': Row(n=1)}", "{'a': 2, 'b': 'y', 'c': Row(n=1)}"]),
    ('expr/column reference', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 2}"]),
    ('expr-replace/column reference', ['a', 'b'], 'struct<a:int,b:string>', [{}, {}], ["{'a': 1, 'b': 'x'}", "{'a': 2, 'b': 'y'}"]),
    pytest.param(*('expr-meta/column reference', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 2}"]), marks=_SAIL_BUG),
    ('expr/arithmetic', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': 2}", "{'a': 2, 'b': 'y', 'c': 3}"]),
    ('expr-replace/arithmetic', ['a', 'b'], 'struct<a:int,b:string>', [{}, {}], ["{'a': 2, 'b': 'x'}", "{'a': 3, 'b': 'y'}"]),
    pytest.param(*('expr-meta/arithmetic', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': 2}", "{'a': 2, 'b': 'y', 'c': 3}"]), marks=_SAIL_BUG),
    ('expr/case when', ['a', 'b', 'c'], 'struct<a:int,b:string,c:string>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': 'small'}", "{'a': 2, 'b': 'y', 'c': 'big'}"]),
    ('expr-replace/case when', ['a', 'b'], 'struct<a:string,b:string>', [{}, {}], ["{'a': 'big', 'b': 'y'}", "{'a': 'small', 'b': 'x'}"]),
    pytest.param(*('expr-meta/case when', ['a', 'b', 'c'], 'struct<a:int,b:string,c:string>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': 'small'}", "{'a': 2, 'b': 'y', 'c': 'big'}"]), marks=_SAIL_BUG),
    ('expr/coalesce', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 2}"]),
    ('expr-replace/coalesce', ['a', 'b'], 'struct<a:int,b:string>', [{}, {}], ["{'a': 1, 'b': 'x'}", "{'a': 2, 'b': 'y'}"]),
    pytest.param(*('expr-meta/coalesce', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 2}"]), marks=_SAIL_BUG),
    ('expr/nested field', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 2}"]),
    ('expr-replace/nested field', ['a', 'b'], 'struct<a:int,b:string>', [{}, {}], ["{'a': 1, 'b': 'x'}", "{'a': 2, 'b': 'y'}"]),
    pytest.param(*('expr-meta/nested field', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 2}"]), marks=_SAIL_BUG),
    ('expr/window function', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 2}"]),
    ('expr-replace/window function', ['a', 'b'], 'struct<a:int,b:string>', [{}, {}], ["{'a': 1, 'b': 'x'}", "{'a': 2, 'b': 'y'}"]),
    ('expr-meta/window function', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': 1}", "{'a': 2, 'b': 'y', 'c': 2}"]),
    ('expr/nondeterministic', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': 0}", "{'a': 2, 'b': 'y', 'c': 0}"]),
    ('expr-replace/nondeterministic', ['a', 'b'], 'struct<a:int,b:string>', [{}, {}], ["{'a': 0, 'b': 'x'}", "{'a': 0, 'b': 'y'}"]),
    pytest.param(*('expr-meta/nondeterministic', ['a', 'b', 'c'], 'struct<a:int,b:string,c:int>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': 0}", "{'a': 2, 'b': 'y', 'c': 0}"]), marks=_SAIL_BUG),
    ('expr/cast of itself', ['a', 'b', 'c'], 'struct<a:int,b:string,c:string>', [{}, {}, {}], ["{'a': 1, 'b': 'x', 'c': '1'}", "{'a': 2, 'b': 'y', 'c': '2'}"]),
    ('expr-replace/cast of itself', ['a', 'b'], 'struct<a:string,b:string>', [{}, {}], ["{'a': '1', 'b': 'x'}", "{'a': '2', 'b': 'y'}"]),
    pytest.param(*('expr-meta/cast of itself', ['a', 'b', 'c'], 'struct<a:int,b:string,c:string>', [{}, {}, {'k': 'v'}], ["{'a': 1, 'b': 'x', 'c': '1'}", "{'a': 2, 'b': 'y', 'c': '2'}"]), marks=_SAIL_BUG),
]
...
def test_metadata_or_expression_result(spark, case, columns, schema, metadata, rows):
    # A timestamp case renders in the session time zone, so it has to be pinned for the expected
...
NAME_RESULTS = [
    ...
    ('add/very long', ['a', ''], ["{'a': 1, '': 9}"]),
    ('rename/very long', [''], ["{'': 1}"]),
    ('replace/very long', [''], ["{'': 9}"]),
    ('metadata/very long', [''], ["{'': 1}"]),
    ('add/emoji', ['a', '😀'], ["{'a': 1, '😀': 9}"]),
    ('rename/emoji', ['😀'], ["{'😀': 1}"]),
    ('replace/emoji', ['😀'], ["{'😀': 9}"]),
    ('metadata/emoji', ['😀'], ["{'😀': 1}"]),
...
def _folding_cases(spark):
    ...
    return {name: (lambda l=left, r=right: named(l).withColumn(r, lit(9))) for name, (left, right) in FOLDING.items()}
