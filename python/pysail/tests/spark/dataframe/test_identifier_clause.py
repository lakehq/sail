...
def test_identifier_variable_column_in_select(spark):
    spark.sql(
...
def test_identifier_variable_column_in_where(spark):
    spark.sql(
...
def test_identifier_variable_table_in_from(spark):
...
def test_identifier_variable_constant_folding(spark):
    spark.sql("CREATE OR REPLACE TEMPORARY VIEW t_id_fold AS SELECT * FROM VALUES (1, 'a'), (2, 'b') AS t(id, name)")
    result = spark.sql(
