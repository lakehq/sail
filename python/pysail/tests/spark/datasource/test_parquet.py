...
def test_parquet_write_with_bloom_filter(spark, tmpdir):
    ...
    # Bloom filter sizing may change across Parquet versions (for example if the
    # writer "folds down" the bloom filter based on inserted values). To keep
    # this test stable, assert that enabling bloom filters increases file size
    # for a dataset with many distinct values.

    fpp = "0.001"
    ndv = "10000"

    df_one_col = spark.range(0, 10_000).selectExpr("id as id")

    path = str(tmpdir / "bloom_filter_off_explicit")
    (
        df_one_col.write.option("bloom_filter_on_write", "false")
        .option("bloom_filter_fpp", fpp)
        .option("bloom_filter_ndv", ndv)
        .parquet(path)
    )
    size_off_one = size(path)

    path = str(tmpdir / "bloom_filter_off_implicit")
    (
        df_one_col.write.option("bloom_filter_fpp", fpp)
        .option("bloom_filter_ndv", ndv)
        .parquet(path)
    )
    assert size(path) <= size_off_one + 1024  # noqa: PLR2004

    path = str(tmpdir / "bloom_filter_on")
    (
        df_one_col.write.option("bloom_filter_on_write", "true")
        .option("bloom_filter_fpp", fpp)
        .option("bloom_filter_ndv", ndv)
        .parquet(path)
    )
    assert size(path) > size_off_one + 4096  # noqa: PLR2004

    df_two_cols = spark.range(0, 10_000).selectExpr("id as a", "id as b")

    path = str(tmpdir / "bloom_filter_off_two_cols")
    (
        df_two_cols.write.option("bloom_filter_on_write", "false")
        .option("bloom_filter_fpp", fpp)
        .option("bloom_filter_ndv", ndv)
        .parquet(path)
    )
    size_off_two = size(path)

    path = str(tmpdir / "bloom_filter_on_two_cols")
    (
        df_two_cols.write.option("bloom_filter_on_write", "true")
        .option("bloom_filter_fpp", fpp)
        .option("bloom_filter_ndv", ndv)
        .parquet(path)
    )
    assert size(path) > size_off_two + 4096  # noqa: PLR2004
