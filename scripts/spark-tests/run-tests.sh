...
if [ "${#pytest_args[@]}" -ne 0 ]; then
  ...
  case "$(basename "${VIRTUAL_ENV}")" in
    test-spark.*)
      run_pytest test-connect --pyargs pyspark.sql.tests.connect -n 2 --tb=no -rN
      ...
      run_pytest doctest-functions --doctest-modules --pyargs pyspark.sql.functions -n 2 --tb=no -rN
      ;;
    test-ibis)
