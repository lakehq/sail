...
Sail supports reading and writing data to AWS S3 and S3-compatible object storage services such as MinIO, Cloudflare R2, or Aliyun OSS.
...
You can configure the endpoint and credentials for Aliyun OSS using environment variables so that you can use the `s3` URL scheme to access Aliyun OSS buckets.
Here is an example, where `${bucket}` is the bucket name.
...
export AWS_ENDPOINT="https://${bucket}.s3.oss-cn-hangzhou.aliyuncs.com"
export AWS_VIRTUAL_HOSTED_STYLE_REQUEST="true"
```

You can use the `oss` URL scheme to access Aliyun OSS buckets via the S3-compatible endpoint. We will support the `oss` URL scheme via the native Aliyun OSS endpoint in the future.
You can also use the `https` URL scheme to access Aliyun OSS buckets via the S3-compatible endpoint directly.

<ul>
  <li>
    <code>oss</code> URL scheme
    <ul>
      <li>
        <code><SyntaxText raw="'oss://'<bucket>'/'<key>" /></code>
      </li>
    </ul>
  </li>
  <li>
    <code>https</code> URL scheme
    <ul>
      <li>
        <code
          ><SyntaxText
            raw="'https://'<bucket>'.s3.oss-'<region>'.aliyuncs.com/'<key>"
        /></code>
      </li>
      <li>
        <code
          ><SyntaxText
            raw="'https://s3.oss-'<region>'.aliyuncs.com/'<bucket>'/'<key>"
        /></code>
      </li>
    </ul>
  </li>
...
You can use either long-lived or temporary credentials for Aliyun OSS.

Aliyun OSS S3-compatible endpoints support virtual-hosted-style and path-style URL formats. When you configure `AWS_ENDPOINT`, set `AWS_VIRTUAL_HOSTED_STYLE_REQUEST` to match the endpoint format. The region in the endpoint URL must match the region specified in the `AWS_REGION` environment variable.

The S3-compatible endpoint supports read, write (including multipart upload), list, and delete operations for objects stored in Aliyun OSS.

::: warning

There are a few limitations when using Aliyun OSS S3-compatible access.

- Credential rotation is not supported.
- The S3-compatible endpoint does not support headers such as `If-None-Match` for object uploads. Some lakehouse formats, such as Delta Lake, may not work because they rely on these headers for atomic operations.
