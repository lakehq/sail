...
The S3-compatible endpoint supports read, write (including multipart upload), list, and delete operations for objects stored in Aliyun OSS. Access the data with the `s3://` URI scheme instead of `oss://`.

::: warning

There are a few limitations when using the `s3://` URI scheme to access Aliyun OSS.

- Credential rotation is not supported.
- Because the bucket name is included in the endpoint URL, each Spark Connect server instance can access only one bucket, and you cannot switch buckets at runtime.
