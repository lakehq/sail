...
Sail supports reading and writing data to AWS S3 and S3-compatible object storage services (such as [MinIO](https://min.io/) or Cloudflare R2) using the `s3://`, `s3a://`, `oss://`, or `https://` URI schemes.
...
<ul>
  <li>
    <code>s3</code>, <code>s3a</code>, and <code>oss</code> protocols
    <ul>
      <li>
  ...
  <li>
    HTTPS endpoints for Aliyun OSS S3-compatible requests
    <ul>
      <li>
        <code
          ><SyntaxText
...
You can configure the endpoint and credentials for Aliyun OSS using environment variables. Here is an example, where `${bucket}` is the bucket name.
...
The S3-compatible endpoint supports read, write (including multipart upload), list, and delete operations for objects stored in Aliyun OSS. Access the data with the `oss://`, `s3://`, or HTTPS URI formats.
