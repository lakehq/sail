...
Aliyun OSS only supports virtual-hosted-style requests, so you must include the bucket name in the endpoint URL. The region in the endpoint URL must match the region specified in the `AWS_REGION` environment variable.
