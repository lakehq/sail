...
All remote catalog providers (excluding the [Memory catalog](./memory)) support the following common options for caching database, table, and view listings, and the tables loaded from the catalog. This is particularly useful for reducing the number of requests to external services.
