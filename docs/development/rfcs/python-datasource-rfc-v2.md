...
This RFC documents Python DataSource support in Sail, enabling users to implement custom data sources in Python while leveraging Sail's distributed execution engine. The implementation provides **100% API compatibility** with PySpark 3.5+/4.0+ DataSource API, allowing existing PySpark DataSource implementations to work without modification.
...
Python DataSource support provides a **composable plugin architecture**:
...
Python is the strategic choice for data extensions:

1. **Ecosystem Dominance**: PyArrow, connector-x, DuckDB, boto3, google-cloud-python
2. **PySpark Migration Path**: Reuse existing DataSource implementations with zero code changes
3. **Developer Accessibility**: Python developers outnumber Rust developers ~50:1 in data space
4. **Subprocess Isolation**: Python's architecture enables straightforward crash isolation
...
7. **Process reuse strategy**: When subprocess isolation is implemented (Phase 3), how should worker processes be managed?
...
**Performance Notes:**
- **Control plane**: ~10-50ms per query (schema, partitions)
- **Data plane**: Zero-copy, sub-microsecond overhead
- **GIL contention**: Only during control plane; data transfer releases GIL
- **Parallelism**: Limited by global GIL in MVP; Phase 3 provides N GILs
...
Discovers DataSources via two mechanisms:
...
Sail provides **100% API compatibility** with PySpark 3.5+/4.0+. Existing PySpark DataSources work without modification:
...
| **Phase 2** | Batch Write | Write exec, commit protocol (~550 LOC) | Planned |
| **Phase 3** | Subprocess Isolation | gRPC, shared memory, crash isolation (~800 LOC) | Planned |
| **Phase 4** | Streaming Read | Offset management (~650 LOC) | Planned |
| **Phase 5** | Streaming Write | BatchId protocol (~600 LOC) | Planned |
| **Phase 6** | Performance & Polish | Exotic types, metrics (~250 LOC) | Planned |

**Total Estimated**: ~5,476 lines across 6 incremental deliveries
...
**Q15: What data types are supported in MVP vs future phases?**
...
| Alternative | Pros | Cons | Decision |
|-------------|------|------|----------|
| **Arrow C Data Interface** | Zero-copy, standard | Requires PyArrow | Chosen |
| **Arrow IPC** | Works across processes | Serialization overhead | Subprocess mode |
...
|------|------------|--------|------------|
| **GIL bottleneck** | Medium | High | Subprocess isolation (Phase 3) |
| **Python crash kills Sail** | Low | Critical | Subprocess isolation (Phase 3) |
| **Memory leak at boundary** | Low | Medium | RAII cleanup, integration tests |
| **Arrow version mismatch** | Medium | Medium | Pin arrow-pyarrow, validate at load |
| **PySpark API divergence** | Low | High | Track releases, compatibility tests |
