...
mod tests {
    ...
    fn original_spec() -> PartitionSpec {
        PartitionSpec::builder()
            .with_spec_id(0)
            .add_field_with_id(1, 1000, "x", Transform::Identity)
            .add_field_with_id(2, 1001, "y", Transform::Identity)
            .build()
    }

    fn requested_replacement() -> PartitionSpec {
        PartitionSpec::builder()
            .add_field(3, "z_bucket", Transform::Bucket(8))
            .add_field(1, "x", Transform::Identity)
            .build()
    }

    #[test]
    fn v2_replacement_reuses_historical_field_ids() {
        let replacement = assign_replacement_partition_spec(
            FormatVersion::V2,
            &[original_spec()],
            0,
            1001,
        ...
        assert_eq!(replacement.spec_id(), 1);
        assert_eq!(
            replacement
                .fields()
                .iter()
    ...
    #[test]
    fn v1_replacement_keeps_removed_fields_as_void() {
        let replacement = assign_replacement_partition_spec(
            FormatVersion::V1,
            &[original_spec()],
            0,
        ...
        let current_schema = Schema::builder()
            ...
            .build()
            .expect("valid schema");
        let partition_type = replacement
