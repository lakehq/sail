...
mod tests {
    ...
    fn valid_manifest_metadata() -> Result<HashMap<String, Vec<u8>>, String> {
        ...
        Ok(HashMap::from([
            ...
            ("schema-id".to_string(), b"1".to_vec()),
            ("partition-spec".to_string(), b"[]".to_vec()),
            ("partition-spec-id".to_string(), b"0".to_vec()),
    ...
    #[test]
    fn malformed_manifest_header_values_are_rejected() -> Result<(), String> {
        for (key, value) in [
            ("schema-id", b"not-an-id".as_slice()),
            ("partition-spec-id", b"not-an-id".as_slice()),
            ("format-version", b"9".as_slice()),
            ("content", b"unknown".as_slice()),
        ] {
            let mut metadata = valid_manifest_metadata()?;
            metadata.insert(key.to_string(), value.to_vec());
            assert!(
                ManifestMetadata::parse_from_avro_meta(&metadata).is_err(),
    ...
    #[test]
    fn missing_legacy_manifest_header_values_keep_v1_defaults() -> Result<(), String> {
        let mut metadata = valid_manifest_metadata()?;
        for key in [
            "schema-id",
            "partition-spec-id",
