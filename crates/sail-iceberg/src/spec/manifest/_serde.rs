...
mod tests {
    ...
    #[test]
    fn correctness_manifest_bounds_decode_decimal_and_uuid() {
        ...
        assert_eq!(
            bounds.get(&1),
            Some(&Datum::new(decimal, PrimitiveLiteral::Int128(-1234)))
        );
        assert_eq!(
            bounds.get(&2),
            Some(&Datum::new(
                PrimitiveType::Uuid,
                PrimitiveLiteral::UInt128(uuid),
            ))
        );
    }
}
