...
impl SnapshotUpdateKind {
    fn summary_operation(self, changes: SnapshotChanges) -> Operation {
        match self {
            ...
            Self::CopyOnWriteDelete | Self::CopyOnWrite
                if changes.adds_data_files() && !changes.removes_data_files() =>
            {
                Operation::Append
            }
            Self::CopyOnWriteDelete | Self::CopyOnWrite
                if changes.removes_data_files() && !changes.adds_data_files() =>
            {
                Operation::Delete
            }
            Self::CopyOnWriteDelete | Self::CopyOnWrite => Operation::Overwrite,
        }
    }
...
mod tests {
    ...
    #[test]
    fn copy_on_write_summary_operation_reflects_file_effects() {
        for kind in [
    ...
    #[test]
    fn manifest_min_sequence_uses_live_entries_and_preserves_content() {
    ...
    #[test]
    fn snapshot_producer_writes_v1_snapshot_and_manifest_ids() {
        futures::executor::block_on(async {
