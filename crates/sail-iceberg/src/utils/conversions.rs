...
fn primitive_literal_to_scalar(prim: &PrimitiveLiteral, prim_type: &PrimitiveType) -> ScalarValue {
    ...
    match (prim_type, prim) {
        ...
        (PrimitiveType::Long, PL::Int(v)) => SV::Int64(Some(i64::from(*v))),
        (PrimitiveType::Double, PL::Float(v)) => SV::Float64(Some(f64::from(v.into_inner()))),
        // Date: Int -> Date32
        ...
        // UUID uses its canonical 16-byte Arrow representation.
        (PrimitiveType::Uuid, PL::UInt128(u)) => {
            SV::FixedSizeBinary(16, Some(u.to_be_bytes().to_vec()))
        }
        (PrimitiveType::Fixed(size), PL::Binary(bytes)) => i32::try_from(*size)
            .map(|size| SV::FixedSizeBinary(size, Some(bytes.clone())))
            .unwrap_or_else(|_| SV::LargeBinary(Some(bytes.clone()))),
        (PrimitiveType::Binary, PL::Binary(bytes))
        | (PrimitiveType::Geometry { .. }, PL::Binary(bytes))
        | (PrimitiveType::Geography { .. }, PL::Binary(bytes)) => {
            SV::LargeBinary(Some(bytes.clone()))
        }
