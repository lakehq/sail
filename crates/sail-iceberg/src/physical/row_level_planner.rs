...
use datafusion::logical_expr::{Operator, TableScanBuilder, TableSource};
...
pub(crate) async fn plan_iceberg_row_level_write(
    ...
    match node.command() {
        RowLevelCommand::Delete => {
            plan_iceberg_delete(session, planner, node, physical_inputs).await
        }
        RowLevelCommand::Update => plan_iceberg_update(session, node, physical_inputs).await,
...
async fn plan_iceberg_merge(
    ...
    let (table_url, table) = load_row_level_table(session, node).await?;
    match current_row_level_mode(&table, RowLevelCommand::Merge)? {
        ...
        IcebergRowLevelMode::MergeOnRead => {
            plan_iceberg_merge_on_read(session, node, physical_inputs, table_url, &table)
        }
    }
}
...
async fn plan_iceberg_delete(
    ...
    match current_row_level_mode(&table, RowLevelCommand::Delete)? {
        ...
        IcebergRowLevelMode::MergeOnRead => {
            plan_iceberg_delete_merge_on_read(session, planner, node, table_url, &table).await
        }
    }
}
...
async fn plan_iceberg_delete_merge_on_read(
    ...
    let target_scan = datafusion::logical_expr::LogicalPlan::TableScan(
    ...
    let delete_plan = LogicalPlanBuilder::from(target_scan)
        ...
        .build()?;
    let delete_plan = session.optimize(&delete_plan)?;
