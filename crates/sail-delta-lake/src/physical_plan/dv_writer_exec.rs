...
impl DeletionVectorRowsWriterExec {
    pub fn new(
        ...
        let partition_count = input
            .output_partitioning()
            .partition_count()
            .max(adds_input.output_partitioning().partition_count())
            .max(1);
        let cache = Arc::new(PlanProperties::new(
            EquivalenceProperties::new(schema),
            Partitioning::UnknownPartitioning(partition_count),
...
impl ExecutionPlan for DeletionVectorRowsWriterExec {
    ...
    fn execute(
        ...
        let future = async move {
            ...
            let mut rows_by_path: HashMap<String, Vec<u64>> = HashMap::new();
            ...
            while let Some(batch_result) = adds_stream.next().await {
                ...
                for add in adds {
                    if rows_by_path.contains_key(&add.path) {
