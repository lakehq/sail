...
use sail_common_datafusion::datasource::{MERGE_FILE_COLUMN, MERGE_ROW_INDEX_COLUMN, UpdateInfo};
...
pub fn expand_update_node(mut info: UpdateInfo) -> Result<LogicalPlan> {
    ...
    Ok(LogicalPlan::Extension(Extension {
        node: Arc::new(expand_update(info, MERGE_FILE_COLUMN, row_index_column)?),
    }))
}
