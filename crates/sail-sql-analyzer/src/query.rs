...
fn from_ast_projection_named_expression(
    expr: NamedExpr,
    auto_generated_alias_metadata: bool,
) -> SqlResult<spec::Expr> {
    let has_alias = expr.alias.is_some();
    let expr = from_ast_named_expression(expr)?;
    if has_alias || !auto_generated_alias_metadata || !needs_auto_generated_alias_metadata(&expr) {
        Ok(expr)
    } else {
...
fn needs_auto_generated_alias_metadata(expr: &spec::Expr) -> bool {
    !matches!(
        expr,
        spec::Expr::Literal(_)
...
fn with_auto_generated_alias_metadata(expr: spec::Expr) -> spec::Expr {
    const AUTO_GENERATED_ALIAS: (&str, &str) = ("__autoGeneratedAlias", "\"true\"");
    ...
    match expr {
        spec::Expr::Alias {
            ...
            if !metadata
                .iter()
                .any(|(key, _)| key == AUTO_GENERATED_ALIAS.0)
            {
                metadata.push((
                    AUTO_GENERATED_ALIAS.0.to_string(),
                    AUTO_GENERATED_ALIAS.1.to_string(),
                ));
            }
            spec::Expr::Alias {
                expr,
...
pub(crate) fn from_ast_query(query: Query) -> SqlResult<spec::QueryPlan> {
    from_ast_query_with_auto_generated_alias_metadata(query, true)
}

pub(crate) fn from_ast_expression_subquery(query: Query) -> SqlResult<spec::QueryPlan> {
    from_ast_query_with_auto_generated_alias_metadata(query, false)
}

fn from_ast_query_with_auto_generated_alias_metadata(
    query: Query,
    auto_generated_alias_metadata: bool,
    ...
    let plan = from_ast_query_body(*body, auto_generated_alias_metadata)?;
    ...
    {
        let ctes =
            from_ast_with_auto_generated_alias_metadata(ctes, auto_generated_alias_metadata)?;
...
fn from_ast_query_term(
    term: QueryTerm,
    auto_generated_alias_metadata: bool,
) -> SqlResult<spec::QueryPlan> {
    match term {
        QueryTerm::Select(select) => from_ast_query_select(*select, auto_generated_alias_metadata),
        QueryTerm::Table(_, name) => from_ast_query_table(name),
        QueryTerm::Values(values) => from_ast_values(values),
        QueryTerm::Nested(_, query, _) => {
            from_ast_query_with_auto_generated_alias_metadata(query, auto_generated_alias_metadata)
        }
    }
}

fn from_ast_query_select(
    select: QuerySelect,
    auto_generated_alias_metadata: bool,
) -> SqlResult<spec::QueryPlan> {
    let QuerySelect {
    ...
    let tables = from
        ...
        .unwrap_or_default();
    let plan =
        from_ast_tables_with_auto_generated_alias_metadata(tables, auto_generated_alias_metadata)?;
    ...
    let projection = projection
        .into_items()
        .map(|x| from_ast_projection_named_expression(x, auto_generated_alias_metadata))
        .collect::<SqlResult<_>>()?;
...
fn from_ast_query_body(
    body: QueryBody,
    auto_generated_alias_metadata: bool,
) -> SqlResult<spec::QueryPlan> {
    match body {
        QueryBody::Term(term) => from_ast_query_term(*term, auto_generated_alias_metadata),
        QueryBody::SetOperation {
            ...
            let left = from_ast_query_body(*left, auto_generated_alias_metadata)?;
            let right = from_ast_query_body(*right, auto_generated_alias_metadata)?;
...
fn from_ast_table(
    ...
    if lateral.is_some() {
        ...
        match &table {
            ...
            TableFactor::Query { .. } => query_plan_with_lateral_subquery(
                input,
                table,
                None,
                spec::JoinType::Cross,
                auto_generated_alias_metadata,
            ),
        ...
        query_plan_with_table_factor(input, table, joins, auto_generated_alias_metadata)
    }
}

pub fn from_ast_tables(tables: Vec<TableWithJoins>) -> SqlResult<spec::QueryPlan> {
    from_ast_tables_with_auto_generated_alias_metadata(tables, true)
}

fn from_ast_tables_with_auto_generated_alias_metadata(
    tables: Vec<TableWithJoins>,
    auto_generated_alias_metadata: bool,
) -> SqlResult<spec::QueryPlan> {
    let plan = tables
        .into_iter()
        .try_fold(None, |plan, table| {
            from_ast_table(plan, table, auto_generated_alias_metadata).map(Some)
        })?
        .unwrap_or_else(|| {
...
pub fn from_ast_table_factor_with_joins(
    ...
    from_ast_table_factor_with_joins_and_auto_generated_alias_metadata(table, joins, true)
}

fn from_ast_table_factor_with_joins_and_auto_generated_alias_metadata(
    table: TableFactor,
    joins: Vec<TableJoin>,
    auto_generated_alias_metadata: bool,
) -> SqlResult<spec::QueryPlan> {
    let plan = from_ast_table_factor(table, auto_generated_alias_metadata)?;
    let plan = joins.into_iter().try_fold(plan, |plan, join| {
        query_plan_with_join(plan, join, auto_generated_alias_metadata)
    })?;
    Ok(plan)
}
...
fn from_ast_table_factor(
    ...
    match table {
        ...
        TableFactor::Query {
            ...
            let plan = from_ast_query_with_auto_generated_alias_metadata(
                query,
                auto_generated_alias_metadata,
            )?;
        ...
        TableFactor::Nested {
            ...
            let plan = from_ast_table(None, *table, auto_generated_alias_metadata)?;
            let plan = query_plan_with_table_modifiers(plan, modifiers)?;
            query_plan_with_table_alias(plan, alias)
        }
...
pub fn from_ast_with(
    ...
    from_ast_with_auto_generated_alias_metadata(ctes, true)
}

fn from_ast_with_auto_generated_alias_metadata(
    ctes: Sequence<NamedQuery, Comma>,
    auto_generated_alias_metadata: bool,
) -> SqlResult<Vec<(spec::Identifier, spec::QueryPlan)>> {
    ctes.into_items()
...
fn query_plan_with_table_factor(
    ...
    let right = from_ast_table_factor_with_joins_and_auto_generated_alias_metadata(
        table,
        joins,
        auto_generated_alias_metadata,
    )?;
...
fn query_plan_with_join(
    ...
    if lateral.is_some() {
        return match &right {
            ...
            TableFactor::Query { .. } => {
                ...
                query_plan_with_lateral_subquery(
                    Some(left),
                    right,
                    criteria,
                    join_type,
    ...
    let right = from_ast_table_factor(right, auto_generated_alias_metadata)?;
    let join_type = match operator {
        None | Some(JoinOperator::Inner(_)) => spec::JoinType::Inner,
        Some(JoinOperator::LeftOuter(_, _)) | Some(JoinOperator::Left(_)) => {
            spec::JoinType::LeftOuter
