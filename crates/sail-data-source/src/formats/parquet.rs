...
impl TableFormat for ParquetTableFormat {
    ...
    async fn create_provider(&self, info: SourceInfo<'_>) -> Result<Arc<dyn TableProvider>> {
        ...
        let options = resolver.resolve_parquet_read_options(info.options)?;
        let listing_options =
...
impl TableWriter for ParquetTableFormat {
    fn name(&self) -> &str {
        "parquet"
    }
    ...
    fn create_writer(&self, info: SinkInfo<'_>) -> Result<Arc<dyn FileFormatFactory>> {
        let resolver = DataSourceOptionsResolver::new(info.ctx);
        let options = resolver.resolve_parquet_write_options(info.options)?;
