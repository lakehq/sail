...
mod tests {
    ...
    fn nested_schema(string_type: DataType, binary_type: DataType) -> Schema {
        Schema::new(vec![
            Field::new("key", string_type.clone(), true),
            Field::new(
                "payload",
                DataType::Struct(
                    vec![
                        Field::new("label", string_type.clone(), true),
                        Field::new(
                            "aliases",
                            DataType::List(Arc::new(Field::new_list_field(string_type, true))),
                            true,
                        ),
                        Field::new("raw", binary_type, true),
                    ]
                    .into(),
                ),
                true,
    ...
    #[test]
    fn normalizes_mixed_nested_string_types_before_schema_merge() -> Result<()> {
        let normalizer = ParquetSchemaNormalizer {
            binary_as_string: false,
            force_view_types: true,
        };
        let schemas = [
            nested_schema(DataType::Utf8, DataType::Binary),
            nested_schema(DataType::LargeUtf8, DataType::LargeBinary),
            nested_schema(DataType::Utf8View, DataType::BinaryView),
        ]
        .map(|schema| normalizer.normalize_schema(schema));

        let merged = Schema::try_merge(schemas)?;
        let expected = nested_schema(DataType::Utf8View, DataType::BinaryView);
        assert_eq!(merged, expected);
        Ok(())
    }

    #[test]
    fn converts_binary_to_string_before_forcing_view_types() {
        let normalizer = ParquetSchemaNormalizer {
        ...
        let normalized =
            normalizer.normalize_schema(nested_schema(DataType::Binary, DataType::BinaryView));
        let expected = nested_schema(DataType::Utf8View, DataType::Utf8View);
