...
fn path_ends_with_ignore_case(path: &str, suffix: &str) -> bool {
...
fn resolve_listing_file_extension(
    ...
    for (_, object_metas) in file_groups {
        for object_meta in object_metas {
            ...
            if path_ends_with_ignore_case(path.as_ref(), extension_with_compression) {
                count_with_compression += 1;
            } else if path_ends_with_ignore_case(path.as_ref(), file_extension) {
...
fn infer_listing_file_extension(
    ...
    for (_, object_metas) in file_groups {
        for object_meta in object_metas {
            let path = &object_meta.location;
            if path_ends_with_ignore_case(path.as_ref(), file_extension) {
                base_count += 1;
            }
            for c in [
                ...
                if path_ends_with_ignore_case(path.as_ref(), &candidate) {
    ...
    Ok(list
        .try_filter(move |meta| {
            ...
            let extension_with_compression_match = extension_with_compression
                .is_some_and(|ext| path_ends_with_ignore_case(path.as_ref(), ext));
            let extension_match = path_ends_with_ignore_case(path.as_ref(), file_extension)
            ...
            let extension_match = if !extension_match && extension_with_compression.is_none() {
                ...
                .any(|c| {
                    ...
                    path_ends_with_ignore_case(path.as_ref(), &candidate)
                })
            } else {
                extension_match
            };
