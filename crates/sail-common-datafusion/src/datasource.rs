...
pub struct SinkInfo {
    ...
    pub options: Vec<OptionLayer>,
    /// The logical schema of the writer's input, if available. Unlike the
    /// physical `input.schema()`, this schema can preserve arrow field
    /// metadata that the physical planner may strip (e.g. metadata attached
    /// via `Expr::Alias::with_metadata`). Table formats can use this to
    /// recover column-level metadata such as `delta.generationExpression`.
