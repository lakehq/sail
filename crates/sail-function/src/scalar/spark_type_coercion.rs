use std::sync::Arc;

use datafusion::arrow::datatypes::{DataType, FieldRef};

fn merge_compatible_field(
    left: &FieldRef,
    right: &FieldRef,
    allow_null_widening: bool,
) -> Option<FieldRef> {
    let data_type = resolve_spark_view_compatible_type(
        left.data_type(),
        right.data_type(),
        allow_null_widening,
    )?;
    Some(Arc::new(
        left.as_ref()
            .clone()
            .with_data_type(data_type)
            .with_nullable(left.is_nullable() || right.is_nullable()),
    ))
}

fn merge_struct_field(
    left: &FieldRef,
    right: &FieldRef,
    allow_null_widening: bool,
) -> Option<FieldRef> {
    if left.name() != right.name() {
        return None;
    }
    merge_compatible_field(left, right, allow_null_widening)
}

fn merge_map_entries(
    left: &FieldRef,
    right: &FieldRef,
    allow_null_widening: bool,
) -> Option<FieldRef> {
    let (DataType::Struct(left_fields), DataType::Struct(right_fields)) =
        (left.data_type(), right.data_type())
    else {
        return None;
    };
    if left_fields.len() != right_fields.len() {
        return None;
    }

    let fields = left_fields
        .iter()
        .zip(right_fields)
        .map(|(left, right)| merge_compatible_field(left, right, allow_null_widening))
        .collect::<Option<Vec<_>>>()?;
    Some(Arc::new(
        left.as_ref()
            .clone()
            .with_data_type(DataType::Struct(fields.into()))
            .with_nullable(left.is_nullable() || right.is_nullable()),
    ))
}

fn spark_array_element(data_type: &DataType) -> Option<(&FieldRef, bool)> {
    match data_type {
        DataType::List(field) | DataType::ListView(field) | DataType::FixedSizeList(field, _) => {
            Some((field, false))
        }
        DataType::LargeList(field) | DataType::LargeListView(field) => Some((field, true)),
        _ => None,
    }
}

/// Finds a common Arrow type when two Spark values differ only in physical encoding.
///
/// Spark has one logical string type, one logical binary type, and one logical array type. View,
/// offset-width, and Arrow array-container differences are therefore recursively coerced to a
/// common representation before custom functions compare or combine their inputs.
pub(crate) fn spark_view_compatible_type(left: &DataType, right: &DataType) -> Option<DataType> {
    resolve_spark_view_compatible_type(left, right, true)
}

/// Checks the same physical encoding equivalence without treating `Null` as another type.
pub(crate) fn spark_view_equivalent_type(left: &DataType, right: &DataType) -> Option<DataType> {
    resolve_spark_view_compatible_type(left, right, false)
}

fn resolve_spark_view_compatible_type(
    left: &DataType,
    right: &DataType,
    allow_null_widening: bool,
) -> Option<DataType> {
    if left == right {
        return Some(left.clone());
    }

    match (left, right) {
        (
            DataType::Utf8 | DataType::LargeUtf8 | DataType::Utf8View,
            DataType::Utf8 | DataType::LargeUtf8 | DataType::Utf8View,
        ) => {
            return Some(
                if matches!(left, DataType::LargeUtf8) || matches!(right, DataType::LargeUtf8) {
                    DataType::LargeUtf8
                } else {
                    DataType::Utf8
                },
            );
        }
        (
            DataType::Binary | DataType::LargeBinary | DataType::BinaryView,
            DataType::Binary | DataType::LargeBinary | DataType::BinaryView,
        ) => {
            return Some(
                if matches!(left, DataType::LargeBinary) || matches!(right, DataType::LargeBinary) {
                    DataType::LargeBinary
                } else {
                    DataType::Binary
                },
            );
        }
        (DataType::Null, other) | (other, DataType::Null) if allow_null_widening => {
            return Some(other.clone());
        }
        _ => {}
    }

    if let (Some((left_field, left_large)), Some((right_field, right_large))) =
        (spark_array_element(left), spark_array_element(right))
    {
        let field = merge_compatible_field(left_field, right_field, allow_null_widening)?;
        return Some(if left_large || right_large {
            DataType::LargeList(field)
        } else {
            DataType::List(field)
        });
    }

    match (left, right) {
        (DataType::Struct(left_fields), DataType::Struct(right_fields))
            if left_fields.len() == right_fields.len() =>
        {
            let fields = left_fields
                .iter()
                .zip(right_fields)
                .map(|(left, right)| merge_struct_field(left, right, allow_null_widening))
                .collect::<Option<Vec<_>>>()?;
            Some(DataType::Struct(fields.into()))
        }
        (DataType::Map(left_entries, left_sorted), DataType::Map(right_entries, right_sorted))
            if left_sorted == right_sorted =>
        {
            Some(DataType::Map(
                merge_map_entries(left_entries, right_entries, allow_null_widening)?,
                *left_sorted,
            ))
        }
        (
            DataType::Dictionary(left_key, left_value),
            DataType::Dictionary(right_key, right_value),
        ) if left_key == right_key => Some(DataType::Dictionary(
            left_key.clone(),
            Box::new(resolve_spark_view_compatible_type(
                left_value,
                right_value,
                allow_null_widening,
            )?),
        )),
        _ if left.equals_datatype(right) => Some(left.clone()),
        _ => None,
    }
}

#[cfg(test)]
mod tests {
    use datafusion::arrow::datatypes::Field;
    use datafusion_common::{Result, internal_err};

    use super::*;

    #[test]
    fn merges_view_encodings_recursively() -> Result<()> {
        let view = DataType::List(Arc::new(Field::new(
            "element",
            DataType::Struct(
                vec![
                    Field::new("text", DataType::Utf8View, true),
                    Field::new("bytes", DataType::BinaryView, true),
                ]
                .into(),
            ),
            true,
        )));
        let regular = DataType::List(Arc::new(Field::new(
            "item",
            DataType::Struct(
                vec![
                    Field::new("text", DataType::Utf8, true),
                    Field::new("bytes", DataType::Binary, true),
                ]
                .into(),
            ),
            true,
        )));

        let Some(merged) = spark_view_compatible_type(&view, &regular) else {
            return internal_err!("view and offset arrays must be compatible");
        };
        let DataType::List(element) = merged else {
            return internal_err!("merged type must remain a list");
        };
        let DataType::Struct(fields) = element.data_type() else {
            return internal_err!("list element must remain a struct");
        };
        assert_eq!(element.name(), "element");
        assert_eq!(fields[0].data_type(), &DataType::Utf8);
        assert_eq!(fields[1].data_type(), &DataType::Binary);
        Ok(())
    }

    #[test]
    fn ignores_map_wrapper_field_names() {
        let view = DataType::Map(
            Arc::new(Field::new(
                "entries",
                DataType::Struct(
                    vec![
                        Field::new("key", DataType::Utf8View, false),
                        Field::new("value", DataType::BinaryView, true),
                    ]
                    .into(),
                ),
                false,
            )),
            false,
        );
        let offset = DataType::Map(
            Arc::new(Field::new(
                "key_value_pairs",
                DataType::Struct(
                    vec![
                        Field::new("map_key", DataType::Utf8, false),
                        Field::new("map_value", DataType::Binary, true),
                    ]
                    .into(),
                ),
                false,
            )),
            false,
        );
        let expected = DataType::Map(
            Arc::new(Field::new(
                "entries",
                DataType::Struct(
                    vec![
                        Field::new("key", DataType::Utf8, false),
                        Field::new("value", DataType::Binary, true),
                    ]
                    .into(),
                ),
                false,
            )),
            false,
        );

        assert_eq!(spark_view_compatible_type(&view, &offset), Some(expected));
    }

    #[test]
    fn requires_spark_struct_field_names_to_match() {
        let left = DataType::Struct(vec![Field::new("left", DataType::Utf8View, true)].into());
        let right = DataType::Struct(vec![Field::new("right", DataType::Utf8, true)].into());

        assert_eq!(spark_view_compatible_type(&left, &right), None);
    }

    #[test]
    fn canonicalizes_arrow_array_containers() {
        let fixed =
            DataType::FixedSizeList(Arc::new(Field::new("item", DataType::Utf8View, false)), 2);
        let view = DataType::ListView(Arc::new(Field::new("element", DataType::Utf8, true)));

        let expected = DataType::List(Arc::new(Field::new("item", DataType::Utf8, true)));
        assert_eq!(spark_view_compatible_type(&fixed, &view), Some(expected));
    }

    #[test]
    fn equivalence_does_not_widen_null() {
        assert_eq!(
            spark_view_compatible_type(&DataType::Null, &DataType::Utf8View),
            Some(DataType::Utf8View)
        );
        assert_eq!(
            spark_view_equivalent_type(&DataType::Null, &DataType::Utf8View),
            None
        );
    }
}
