...
impl ScalarUDFImpl for SparkDateFormat {
    ...
    // `override def nullable: Boolean = true` (datetimeExpressions.scala). Deriving from the
    // inputs would report `false` where Spark reports `true` — the unsound direction.
