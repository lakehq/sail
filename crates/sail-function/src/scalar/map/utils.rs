...
mod tests {
    ...
    fn int32_utf8_inputs() -> (ArrayRef, ArrayRef) {
        (
    ...
    #[test]
    fn duplicate_map_key_errors_by_default() -> Result<()> {
        let (keys, values) = int32_utf8_inputs();
        let offsets = [0, 3];

        let Err(error) = map_from_keys_values_offsets_nulls(
            &keys, &values, &offsets, &offsets, None, None, false,
        ) else {
            return internal_err!("duplicate keys should fail under EXCEPTION policy");
        };
        let error = error.to_string();

        assert!(error.contains("[DUPLICATED_MAP_KEY]"), "{error}");
        assert!(error.contains("spark.sql.mapKeyDedupPolicy"), "{error}");
        Ok(())
    }

    #[test]
    fn last_win_preserves_first_key_position_and_last_value() -> Result<()> {
        let (keys, values) = int32_utf8_inputs();
        let offsets = [0, 3];
        ...
        let Some(actual_keys) = map.keys().as_any().downcast_ref::<Int32Array>() else {
            return internal_err!("map keys should be Int32");
        };
        let Some(actual_values) = map.values().as_any().downcast_ref::<StringArray>() else {
            return internal_err!("map values should be Utf8");
        };

        assert_eq!(actual_keys.values(), &[1, 2]);
        assert_eq!(
            actual_values.iter().collect::<Vec<_>>(),
            [Some("c"), Some("b")]
        );
        Ok(())
    }
