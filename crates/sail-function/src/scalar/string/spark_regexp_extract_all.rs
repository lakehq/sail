    ...
    P: StringArrayType<'pattern>,
{
    ...
    let idx_len = idx.len();
    let pattern_scalar = (pattern_len == 1 && pattern.is_valid(0))
        .then(|| parse_regex(SparkRegexpExtract::NAME, pattern.value(0)))
        .transpose()?;
    let idx_scalar = (idx_len == 1 && idx.is_valid(0)).then(|| idx.value(0));
    let pattern_scalar_is_null = pattern_len == 1 && pattern.is_null(0);
    let idx_scalar_is_null = idx_len == 1 && idx.is_null(0);

    let mut builder = StringBuilder::new();
    let mut regex_memo = StrMemo::new();
    for row in 0..values.len() {
        let pattern_index = if pattern_len == 1 { 0 } else { row };
        let idx_index = if idx_len == 1 { 0 } else { row };
        if pattern_scalar_is_null
            || idx_scalar_is_null
            || values.is_null(row)
            || pattern.is_null(pattern_index)
            || idx.is_null(idx_index)
        {
            builder.append_null();
        } else {
            let regex = regex_memo.resolve(
                pattern_scalar.as_ref(),
                || pattern.value(pattern_index),
                |pattern| parse_regex(SparkRegexpExtract::NAME, pattern),
            )?;
            let group_idx = idx_scalar.unwrap_or_else(|| idx.value(idx_index));
    ...
    for row in 0..values.len() {
            ...
            || idx.is_null(idx_index)
        {
            ...
            let regex = regex_memo.resolve(
                pattern_scalar.as_ref(),
                || pattern.value(pattern_index),
                |pattern| parse_regex(SparkRegexpExtractAll::NAME, pattern),
            )?;
            let group_idx = idx_scalar.unwrap_or_else(|| idx.value(idx_index));
...
mod tests {
    use datafusion::arrow::array::{Int64Array, ListArray, StringArray, StringViewArray};
    ...
    #[test]
    fn extract_functions_accept_string_views() -> Result<()> {
        let values: ArrayRef = Arc::new(StringViewArray::from(vec!["a11b", "a22b"]));
        let pattern: ArrayRef = Arc::new(StringViewArray::from(vec!["([0-9]+)"]));
        let idx: ArrayRef = Arc::new(Int64Array::from(vec![1]));

        let first = regexp_extract_inner(&[values.clone(), pattern.clone(), idx.clone()])?;
        let first = first
            .as_any()
            .downcast_ref::<StringArray>()
            .expect("string output");
        ...
        let all = regexp_extract_all_inner(&[values, pattern, idx])?;
        let all = all
            .as_any()
            .downcast_ref::<ListArray>()
            .expect("list output");
        for (row, expected) in ["11", "22"].into_iter().enumerate() {
            let matches = all.value(row);
            let matches = matches
                .as_any()
                .downcast_ref::<StringArray>()
                .expect("string matches");
            assert_eq!(matches.iter().collect::<Vec<_>>(), vec![Some(expected)]);
        }
        Ok(())
    }
}
