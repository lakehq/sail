...
fn mask_return_type(arg_types: &[DataType]) -> Result<DataType> {
...
impl ScalarUDFImpl for SparkMask {
    ...
    fn return_field_from_args(&self, args: ReturnFieldArgs) -> Result<FieldRef> {
        let arg_types = args
            ...
            .collect::<Vec<_>>();
        let data_type = mask_return_type(&arg_types)?;
...
mod return_field_tests {
    ...
    fn test_non_nullable_arguments_still_yield_a_nullable_field() -> Result<()> {
        ...
        let field = SparkMask::new().return_field_from_args(ReturnFieldArgs {
    ...
    fn test_return_type_is_not_the_source_of_truth() {
        assert!(SparkMask::new().return_type(&[DataType::Utf8]).is_err());
    }
}
