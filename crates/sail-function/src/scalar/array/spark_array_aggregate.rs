...
/// logically match the zero value type ignoring nested nullability and Arrow view encoding.
use std::sync::Arc;
...
fn scatter_updates(
    ...
    let update_data = if base.data_type() == updates.data_type() {
        ...
        cast(updates, base.data_type())?.to_data()
    } else {
        return exec_err!(
...
mod tests {
    ...
    #[test]
    fn scatter_updates_casts_view_values_to_accumulator_type() -> Result<()> {
