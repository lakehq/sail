    ...
    fn return_type(&self, arg_types: &[DataType]) -> Result<DataType> {
        ...
        match (array1, array2) {
            ...
            (DataType::List(field1), DataType::List(field2)) => {
                Ok(DataType::List(intersection_element_field(field1, field2)?))
            }

            (DataType::LargeList(field1), DataType::LargeList(field2)) => Ok(DataType::LargeList(
                intersection_element_field(field1, field2)?,
            )),

            (DataType::List(field1), DataType::LargeList(field2))
            | (DataType::LargeList(field1), DataType::List(field2)) => Ok(DataType::LargeList(
                intersection_element_field(field1, field2)?,
            )),
    ...
    fn coerce_types(&self, arg_types: &[DataType]) -> Result<Vec<DataType>> {
        ...
        match (left, right) {
            ...
            _ => {
                ...
                Ok(vec![
                    coerce_list_to_common_element_type(left, &common_type)?,
                    coerce_list_to_common_element_type(right, &common_type)?,
                ])
            }
        }
    }
...
fn coerce_list_to_common_element_type(source: &DataType, common: &DataType) -> Result<DataType> {
    ...
    // Preserve each input's containsNull flag; only the output combines them with AND.
    let field = Arc::new(
        source_field
            .as_ref()
            .clone()
...
mod tests {
    ...
    fn test_array_intersect_coerces_view_and_regular_string_elements() -> Result<()> {
        ...
        let coerced_view = DataType::List(Arc::new(Field::new("element", DataType::Utf8, true)));

        assert_eq!(
            udf.coerce_types(&[view, regular.clone()])?,
            vec![coerced_view, regular]
        );
        Ok(())
    }
