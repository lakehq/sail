...
use datafusion::arrow::array::{
    Array, ArrayRef, Date32Array, Int8Array, Int16Array, Int32Array, Int64Array, ListArray,
    NullBufferBuilder, PrimitiveArray, TimestampMicrosecondArray,
};
use datafusion::arrow::buffer::OffsetBuffer;
use datafusion::arrow::datatypes::{
    ArrowPrimitiveType, DataType, Date32Type, DurationMicrosecondType, Field, FieldRef, Int8Type,
...
fn sequence_primitive_array<T: ArrowPrimitiveType>(array: &ArrayRef) -> Result<&PrimitiveArray<T>> {
    array
        .as_any()
        .downcast_ref::<PrimitiveArray<T>>()
        .ok_or_else(|| {
            exec_datafusion_err!(
                "Spark `sequence` function: expected primitive array {}, got {}",
                T::DATA_TYPE,
                array.data_type()
            )
        })
    ...
    ($name:ident, $native_type:ty, $arrow_type:ty, $array_type:ty, $data_type:expr_2021) => {
        fn $name(args: &[ArrayRef]) -> Result<ArrayRef> {
            let (start_array, stop_array, step_array) = match args {
                [start, stop] => (
                    sequence_primitive_array::<$arrow_type>(start)?,
                    sequence_primitive_array::<$arrow_type>(stop)?,
                    None,
                ),
                [start, stop, step] => (
                    sequence_primitive_array::<$arrow_type>(start)?,
                    sequence_primitive_array::<$arrow_type>(stop)?,
                    Some(sequence_primitive_array::<$arrow_type>(step)?),
                ),
...
fn temporal_step_at(array: &ArrayRef, index: usize) -> Result<Option<TemporalStep>> {
    ...
    match array.data_type() {
        DataType::Interval(IntervalUnit::MonthDayNano) => {
            let value = sequence_primitive_array::<IntervalMonthDayNanoType>(array)?.value(index);
        ...
        DataType::Interval(IntervalUnit::YearMonth) => {
            let value = sequence_primitive_array::<IntervalYearMonthType>(array)?.value(index);
            Ok(Some(TemporalStep::YearMonth {
                months: IntervalYearMonthType::to_months(value),
            }))
        }
        DataType::Interval(IntervalUnit::DayTime) => {
            let value = sequence_primitive_array::<IntervalDayTimeType>(array)?.value(index);
        ...
        DataType::Duration(TimeUnit::Microsecond) => Ok(Some(TemporalStep::DayTime {
            micros: sequence_primitive_array::<DurationMicrosecondType>(array)?.value(index),
        })),
...
fn gen_sequence_date(
    ...
    let (start_array, stop_array, step_array) = match args {
        [start, stop] => (
            sequence_primitive_array::<Date32Type>(start)?,
            sequence_primitive_array::<Date32Type>(stop)?,
            None,
        ),
        [start, stop, step] => (
            sequence_primitive_array::<Date32Type>(start)?,
...
fn gen_sequence_timestamp(
    ...
    let (start_array, stop_array, step_array) = match args {
        [start, stop] => (
            sequence_primitive_array::<TimestampMicrosecondType>(start)?,
            sequence_primitive_array::<TimestampMicrosecondType>(stop)?,
            None,
        ),
        [start, stop, step] => (
            sequence_primitive_array::<TimestampMicrosecondType>(start)?,
...
mod tests {
    ...
    #[test]
    fn primitive_type_mismatch_returns_error() -> Result<()> {
        let start: ArrayRef = Arc::new(Int32Array::from(vec![1]));
        let stop: ArrayRef = Arc::new(Int32Array::from(vec![3]));
        let error = match gen_sequence_i64(&[start, stop]) {
            Ok(_) => return exec_err!("expected primitive type mismatch"),
            Err(error) => error,
        };
        assert!(
            error
                .to_string()
                .contains("expected primitive array Int64, got Int32"),
            "{error}"
        );
        Ok(())
    }
