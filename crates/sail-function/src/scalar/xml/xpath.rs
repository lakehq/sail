...
mod tests {
    ...
    #[test]
    fn returns_text_nodes() -> Result<()> {
        let result = evaluate_xpath("<a><b>b1</b><b>b2</b></a>", "a/b/text()")?;
        assert_eq!(
            result,
    ...
    #[test]
    fn returns_nulls_for_element_nodes() -> Result<()> {
        let result = evaluate_xpath("<a><b>b1</b><b>b2</b></a>", "a/b")?;
        assert_eq!(result, Some(vec![None, None]));
        Ok(())
    }

    #[test]
    fn returns_empty_list_for_no_match() -> Result<()> {
        let result = evaluate_xpath("<a><b>b1</b></a>", "a/c")?;
        assert_eq!(result, Some(vec![]));
        Ok(())
    }

    #[test]
    fn returns_null_for_empty_inputs() -> Result<()> {
        assert_eq!(evaluate_xpath("", "a/b")?, None);
        assert_eq!(evaluate_xpath("<a><b>1</b></a>", "")?, None);
        Ok(())
    }
    ...
    fn errors_for_non_nodeset_result() {
        assert!(matches!(
            evaluate_xpath("<a><b>1</b></a>", "sum(a/b)"),
            Err(error) if error.to_string().contains("NodeList")
        ));
    }
}
