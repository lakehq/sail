...
impl PySparkUdtfPayload {
    ...
    pub fn analyze(
        ...
        Python::attach(|py| -> PyResult<DataType> {
            ...
            for (i, dt) in argument_types.iter().enumerate() {
                ...
                let (is_constant, value_array) = match argument_literals.get(i) {
                    Some(Some(sv)) => {
                        ...
                        let pyarrow_array = array.to_data().to_pyarrow(py)?;
            ...
            let py_arguments = arguments.into_pyobject(py)?;
            ...
            let schema = Schema::from_pyarrow_bound(&schema_pyarrow)?;
