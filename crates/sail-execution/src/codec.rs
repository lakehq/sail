...
use datafusion_proto::physical_plan::{
    AsExecutionPlan, DefaultPhysicalProtoConverter, PhysicalExtensionCodec, PhysicalPlanDecodeContext,
};
...
impl PhysicalExtensionCodec for RemoteExecutionCodec {
    ...
    fn try_encode(&self, node: Arc<dyn ExecutionPlan>, buf: &mut Vec<u8>) -> Result<()> {
        ...
        } else if let Some(delta_metadata_stats_exec) = node.downcast_ref::<DeltaMetadataStatsExec>()
        {
            NodeKind::DeltaMetadataStats(gen::DeltaMetadataStatsExecNode {
        ...
        } else if let Some(delta_remove_actions_exec) = node.downcast_ref::<DeltaRemoveActionsExec>()
        {
