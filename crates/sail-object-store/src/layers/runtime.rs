...
mod tests {
    ...
    #[derive(Debug)]
    struct ConcurrentMultipartUpload {
        barrier: Arc<Barrier>,
    }

    #[async_trait::async_trait]
    impl MultipartUpload for ConcurrentMultipartUpload {
    ...
    async fn multipart_parts_upload_concurrently() {
        let inner = Box::new(ConcurrentMultipartUpload {
            barrier: Arc::new(Barrier::new(2)),
        });
        let mut upload = RuntimeAwareMultipartUpload::new(inner, Handle::current());

        let first = upload.put_part(vec![1].into());
        let second = upload.put_part(vec![2].into());

        let result = timeout(Duration::from_secs(1), try_join(first, second)).await;
        assert!(
            matches!(result, Ok(Ok(_))),
            "multipart uploads failed: {result:?}"
        );
    }
}
