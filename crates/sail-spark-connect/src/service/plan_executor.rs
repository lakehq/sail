...
use sail_plan::{resolve_and_describe_execution_plan, resolve_and_execute_plan};
use tonic::Status;
...
pub(crate) async fn handle_execute_write_stream_operation_start(
    ...
    let (plan, info) = resolve_and_describe_execution_plan(ctx, spark.plan_config()?, plan).await?;
    let stream = service.runner().execute(ctx, plan).await?;
