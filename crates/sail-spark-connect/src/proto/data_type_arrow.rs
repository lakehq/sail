...
impl TryFrom<adt::Field> for sdt::StructField {
    ...
    fn try_from(field: adt::Field) -> SparkResult<sdt::StructField> {
        ...
        let interval_start_field =
            parse_interval_field_metadata(field.metadata(), spec::SAIL_INTERVAL_START_FIELD_KEY)?;
        let interval_end_field =
            parse_interval_field_metadata(field.metadata(), spec::SAIL_INTERVAL_END_FIELD_KEY)?;
        ...
        } else if interval_start_field.is_some() || interval_end_field.is_some() {
        ...
        let metadata = &field
            ...
            .filter(|(k, _)| {
                !k.starts_with("udt.")
                    ...
                    && k.as_str() != spec::SAIL_INTERVAL_START_FIELD_KEY
                    && k.as_str() != spec::SAIL_INTERVAL_END_FIELD_KEY
            })
            .map(|(k, v)| {
