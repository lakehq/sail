...
const SPARK_AUTO_GENERATED_ALIAS_METADATA_KEY: &str = "__autoGeneratedAlias";

fn spark_field_metadata_json(
    metadata: &std::collections::HashMap<String, String>,
) -> SparkResult<Option<String>> {
    if let Some(metadata) = metadata.get(spec::SPARK_METADATA_JSON_KEY) {
        return Ok(Some(metadata.clone()));
    }

    let Some(value) = metadata.get(SPARK_AUTO_GENERATED_ALIAS_METADATA_KEY) else {
        return Ok(None);
    };
    let value = serde_json::from_str::<serde_json::Value>(value)
        .unwrap_or_else(|_| serde_json::Value::String(value.clone()));
    let metadata = serde_json::json!({SPARK_AUTO_GENERATED_ALIAS_METADATA_KEY: value});
...
impl TryFrom<adt::Field> for sdt::StructField {
    ...
    fn try_from(field: adt::Field) -> SparkResult<sdt::StructField> {
        ...
        Ok(sdt::StructField {
            ...
            metadata: spark_field_metadata_json(field.metadata())?,
        })
    }
}
