...
use datafusion_expr::{EmptyRelation, ExprSchemable, LogicalPlanBuilder};
...
impl PlanResolver<'_> {
    ...
    pub(super) async fn resolve_query_plan_with_hidden_fields(
        ...
        let plan = match plan.node {
            ...
            QueryNode::Empty { produce_one_row } => LogicalPlan::EmptyRelation(EmptyRelation {
                produce_one_row,
    ...
    pub(super) fn resolve_expression_types(
        exprs: &[Expr],
        schema: &DFSchema,
