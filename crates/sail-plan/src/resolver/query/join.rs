    ...
    pub(super) async fn resolve_query_join(
        ...
        match (join_type, join_criteria) {
            ...
            (Some(join_type), None) => {
                ...
                if !self.config.cross_join_enabled {
                    return Err(PlanError::AnalysisError(format!(
                        "Detected implicit cartesian product for {join_type:?} join between logical plans. \
                        Join condition is missing or trivial. \
                        Either: use the CROSS JOIN syntax to allow cartesian products between these relations, \
                        or: enable implicit cartesian products by setting the configuration variable \
