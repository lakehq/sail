...
use sail_function::scalar::higher_order::{
    SailArrayAggregate, SailArrayExists, SailArrayFilter, SailArrayForAll, SailArrayTransform,
    SailArrayZipWith, SailMapFilter, SailMapTransformKeys, SailMapTransformValues, SailMapZipWith,
};
...
impl PlanResolver<'_> {
    ...
    async fn resolve_hof_aggregate(
        ...
        let init_arg = args_iter.next().unwrap();
        let merge_lambda = args_iter.next().unwrap();
        ...
        let merge_schema = make_lambda_schema(&[
        ...
        let merge_phys = self
            .ctx
            .create_physical_expr(merge_body_expr, &merge_schema)?;
        ...
        let (finish_phys, finish_param, result_type) = if let Some(finish_lam) = finish_lambda {
            ...
            let finish_schema = make_lambda_schema(&[(fp.clone(), acc_type.clone())])?;
            ...
            let finish_phys = self
                .ctx
                .create_physical_expr(finish_body_expr, &finish_schema)?;
...
fn make_lambda_schema(params: &[(String, DataType)]) -> PlanResult<DFSchemaRef> {
