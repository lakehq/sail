        ...
        match status.kind {
            TableKind::Table {
                ...
                if columns.is_empty() {
                    ...
                    let lake_formats = self.ctx.extension::<LakeFormatRegistry>()?;
                    if let Some(plugin) = lake_formats
                        .get_if_registered(&format)
                    ...
                    let table_format = registry.get_by_name(&format).map_err(|e| {
                        PlanError::invalid(format!(
