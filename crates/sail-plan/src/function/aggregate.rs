...
fn ols_sufficient_stats(input: AggFunctionInput) -> PlanResult<expr::Expr> {
    let args = input.arguments;
    Ok(expr::Expr::AggregateFunction(AggregateFunction {
        func: Arc::new(AggregateUDF::from(OLSSufficientStats::new())),
        params: AggregateFunctionParams {
            args,
            distinct: input.distinct,
            filter: input.filter,
...
fn sgd_gradient_sum(input: AggFunctionInput) -> PlanResult<expr::Expr> {
    let args = input.arguments;
    Ok(expr::Expr::AggregateFunction(AggregateFunction {
        func: Arc::new(AggregateUDF::from(SGDGradientSum::new())),
        params: AggregateFunctionParams {
            args,
            distinct: input.distinct,
            filter: input.filter,
...
fn list_built_in_aggregate_functions() -> Vec<(&'static str, AggFunction)> {
    ...
    vec![
        ...
        ("ols_sufficient_stats", F::custom(ols_sufficient_stats)),
        ("sgd_gradient_sum", F::custom(sgd_gradient_sum)),
