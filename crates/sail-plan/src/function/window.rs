...
use crate::function::aggregate::{approx_percentile_arguments, coerce_string_sum_arguments};
...
fn approx_percentile(input: WinFunctionInput) -> PlanResult<expr::Expr> {
    if input.distinct {
    ...
    Ok(expr::Expr::WindowFunction(Box::new(expr::WindowFunction {
        fun: WindowFunctionDefinition::AggregateUDF(Arc::new(AggregateUDF::from(
            ApproxPercentile::default(),
        ))),
        params: WindowFunctionParams {
            args: arguments,
            partition_by: input.partition_by,
            order_by: input.order_by,
            window_frame: input.window_frame,
            filter: None,
