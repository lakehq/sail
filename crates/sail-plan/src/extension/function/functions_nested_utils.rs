...
use sail_common::utils::new_list_field;
            ...
            if ndim < max_ndim {
                ...
                for _ in 0..(max_ndim - ndim) {
                    ...
                    aligned_array = Arc::new(GenericListArray::<O>::try_new(
                        Arc::new(new_list_field(data_type, true)),
                        offsets,
                        aligned_array,
                        None,
                    )?)
...
pub(crate) fn compare_element_to_list(
    ...
    let res = match element_array_row.data_type() {
        ...
        DataType::LargeList(_) => {
            // compare each element of the from array
            let element_array_row_inner = as_large_list_array(&element_array_row)?.value(0);
        ...
        _ => {
            ...
            if eq {
                cmp::not_distinct(&list_array_row, &element_arr)?
            } else {
                cmp::distinct(&list_array_row, &element_arr)?
            }
        }
    };
...
mod tests {
    ...
    use sail_common::utils::array_into_list_array;
    ...
    fn test_align_array_dimensions() {
        let array1d_1 = Arc::new(ListArray::from_iter_primitive::<Int64Type, _, _>(vec![
            Some(vec![Some(1), Some(2), Some(3)]),
            Some(vec![Some(4), Some(5)]),
        ]));
        let array1d_2 = Arc::new(ListArray::from_iter_primitive::<Int64Type, _, _>(vec![
            Some(vec![Some(6), Some(7), Some(8)]),
        ]));

        let array2d_1 = Arc::new(array_into_list_array(
            Arc::clone(&array1d_1) as ArrayRef,
            true,
        )) as ArrayRef;
        let array2d_2 = Arc::new(array_into_list_array(
            Arc::clone(&array1d_2) as ArrayRef,
            true,
        )) as ArrayRef;

        let res = align_array_dimensions::<i32>(vec![array1d_1.to_owned(), array2d_2.to_owned()])
            .unwrap();
        ...
        let array3d_1 = Arc::new(array_into_list_array(array2d_1, true)) as ArrayRef;
        let array3d_2 = array_into_list_array(array2d_2.to_owned(), true);
        let res = align_array_dimensions::<i32>(vec![array1d_1, Arc::new(array3d_2)]).unwrap();
