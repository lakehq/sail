/// [Credit]: <https://github.com/apache/datafusion/blob/94d178ebe9674669b32ecd7896b5597f49e90791/datafusion/functions-nested/src/make_array.rs>
use std::{any::Any, sync::Arc};

use datafusion::arrow::array::{
    make_array as arrow_make_array, new_null_array, Array, ArrayData, ArrayRef, Capacities,
    GenericListArray, MutableArrayData, NullArray, OffsetSizeTrait,
};
use datafusion::arrow::buffer::OffsetBuffer;
use datafusion::arrow::datatypes::DataType::{self, LargeList, List, Null};
use datafusion_common::{internal_err, plan_err, Result};
use datafusion_expr::type_coercion::binary::comparison_coercion;
use datafusion_expr::{ColumnarValue, ScalarUDFImpl, Signature, TypeSignature, Volatility};
use sail_common::utils::{array_into_list_array, new_list_field};

use crate::extension::function::functions_nested_utils::make_scalar_function;

#[derive(Debug)]
pub struct MakeNullableArray {
    signature: Signature,
    aliases: Vec<String>,
}

impl Default for MakeNullableArray {
    fn default() -> Self {
        Self::new()
    }
}

impl MakeNullableArray {
    pub fn new() -> Self {
        Self {
            ...
            aliases: vec![String::from("make_nullable_list")],
        }
    }
}

impl ScalarUDFImpl for MakeNullableArray {
    ...
    fn return_type(&self, arg_types: &[DataType]) -> Result<DataType> {
        match arg_types.len() {
            ...
            _ => {
                ...
                Ok(List(Arc::new(new_list_field(expr_type, true))))
            }
        }
    }
...
pub(super) fn empty_array_type() -> DataType {
    DataType::List(Arc::new(new_list_field(DataType::Int64, true)))
}
...
pub(crate) fn make_array_inner(arrays: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    match data_type {
        ...
        Null => {
            ...
            Ok(Arc::new(array_into_list_array(array, true)))
        }
