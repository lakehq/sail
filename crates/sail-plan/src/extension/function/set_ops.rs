/// [Credit]: <https://github.com/apache/datafusion/blob/94d178ebe9674669b32ecd7896b5597f49e90791/datafusion/functions-nested/src/set_ops.rs>
use std::any::Any;
...
use datafusion::arrow::array::{
    new_empty_array, Array, ArrayRef, GenericListArray, OffsetSizeTrait,
};
use datafusion::arrow::buffer::OffsetBuffer;
use datafusion::arrow::compute;
use datafusion::arrow::datatypes::DataType::{self, FixedSizeList, LargeList, List, Null};
...
use crate::extension::function::make_nullable_array::{empty_array_type, make_array_inner};
...
impl ScalarUDFImpl for ArrayDistinct {
    ...
    fn return_type(&self, arg_types: &[DataType]) -> Result<DataType> {
        match &arg_types[0] {
            List(field) | FixedSizeList(field, _) => Ok(List(Arc::new(new_list_field(
                field.data_type().clone(),
                true,
            )))),
            LargeList(field) => Ok(LargeList(Arc::new(new_list_field(
                field.data_type().clone(),
                true,
            )))),
...
fn generic_set_lists<OffsetSize: OffsetSizeTrait>(
    ...
    if matches!(l.value_type(), Null) {
        let field = Arc::new(new_list_field(r.value_type(), true));
        return general_array_distinct::<OffsetSize>(r, &field);
    } else if matches!(r.value_type(), Null) {
        let field = Arc::new(new_list_field(l.value_type(), true));
